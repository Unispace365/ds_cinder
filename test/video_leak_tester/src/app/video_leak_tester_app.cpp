...
#include "video_leak_tester_app.h"
...
video_leak_tester_app::video_leak_tester_app()
	: ds::App()
...
void video_leak_tester_app::setupServer(){
...
void video_leak_tester_app::onAppEvent(const ds::Event& in_e){
	if(in_e.mWhat == RequestAppExitEvent::WHAT()){
		quit();
	} 
}

void video_leak_tester_app::onKeyDown(ci::app::KeyEvent event){
	using ci::app::KeyEvent;

	
}

void video_leak_tester_app::fileDrop(ci::app::FileDropEvent event){
...
CINDER_APP(downstream::video_leak_tester_app, ci::app::RendererGl(ci::app::RendererGl::Options().msaa(4)),
