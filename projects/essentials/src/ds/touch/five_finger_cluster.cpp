...
namespace ds { namespace ui {
	...
	void FiveFingerCluster::setClusterUpdateCallback(
		const std::function<void(const ds::ui::TouchInfo::Phase, const Cluster&)>& func) {
		...
		if (ti.mPhase == ds::ui::TouchInfo::Added) {
			...
			if (clustyMcClustClust.mTriggerable) {
				...
				if (clustyMcClustClust.mBoundingBox.getWidth() < mBoundingBoxSize &&
					...
					(Poco::Timestamp().epochMicroseconds() - clustyMcClustClust.mInitialTouchTime) >
						mTriggerTime * 1000000) {
