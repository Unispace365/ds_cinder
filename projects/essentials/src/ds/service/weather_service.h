...
namespace ds { namespace weather {
	...
	struct WeatherSettings {
		WeatherSettings(){};
		WeatherSettings(const std::string& name, const std::string& query, const std::string& apiKey,
						const std::string& units = "metric", const double requeryTime = 60.0)
		  : mName(name)
		  , mQuery(query)
		  , mApiKey(apiKey)
		  , mUnits(units)
		  , mRequeryTime(requeryTime) {}

		/// The name of this weather result, which will be the name of the ContentModel child on mEngine.mContent
		std::string mName;

		/// A query string for the location, such as q=London,uk or zip=97202,us or lat=123&lon=45 or id=12345 (see
		/// openweathermap for id's)
		std::string mQuery;
		...
		/// Recommend a minimum of 60 seconds, the underlying weather only updates once an hour, so more often is
		/// useless
		double mRequeryTime;
	};
	/**
	...
	*
	* Saves to a ContentModelRef delivered to mEngine.mContent as a child with a name set from the settings with this
	format:
	...
	*/
	class WeatherService {
	  ...
	  private:
		...
		void mapWeatherImages(const int& weatherCode, const std::string& iconCode, std::string& outImage,
							  std::wstring& outEmoji);
		void addAttributeParam(ci::XmlTree& node, ds::model::ContentModelRef& model, std::string attrLabel,
