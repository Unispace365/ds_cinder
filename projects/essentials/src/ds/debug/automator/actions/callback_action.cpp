...
namespace ds { namespace debug {
	...
	BaseAction* CallbackActionFactory::build(std::vector<int>& freeList, ds::ui::SpriteEngine& engine,
	 ...
	 */
	CallbackAction::CallbackAction(std::vector<int>& freeList, ds::ui::SpriteEngine& engine, const ci::Rectf& frame,
								   const std::function<void(void)>& callback, const float minTime, const float maxTime)
