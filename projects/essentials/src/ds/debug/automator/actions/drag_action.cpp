...
namespace ds { namespace debug {
	...
	BaseAction* DragActionFactory::build(std::vector<int>& freeList, ds::ui::SpriteEngine& engine,
	...
	bool DragAction::update(float dt) {
		...
		if (mTotal >= mLimit) {
			...
			for (int i = 0; i < mNumberOfFingers; ++i) {
				touches.push_back(
					ci::app::TouchEvent::Touch(mTouchPos[i], mPreviousTouch[i], mInUseList[i], dt, nullptr));
			}
			...
			if (mUpdateTimeTotal >= mUpdateTime) {
				...
				for (int i = 0; i < mNumberOfFingers; ++i) {
					...
					touches.push_back(
						ci::app::TouchEvent::Touch(mTouchPos[i], mTouchPos[i], mInUseList[i], dt, nullptr));
				}
	...
	void DragAction::setup(float limit, int numberOfFingers) {
		...
		float	 radius	  = 20.0f;
		ci::vec2 touchPos = ci::vec2(mFrame.getX1() + ci::randFloat(0.0f, mFrame.getWidth()),
									 mFrame.getY1() + ci::randFloat(0.0f, mFrame.getHeight()));
		...
		for (auto it = mInUseList.begin(), it2 = mInUseList.end(); it != it2; ++it) {
			...
			ci::vec2 nTouchPos = touchPos + ci::vec2(cos(angle) * radius - sin(angle) * radius,
