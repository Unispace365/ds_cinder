...
const std::string FadeFrag = R"FRAG(
...
const std::string FadeVert = R"VERT(
#version 150
...
void ScrollArea::checkBounds(const bool immediate) {
	...
	if (doTween) {
		if (immediate) {
			...
			callAfterDelay(
				[this, tweenDestination]() {
					mWillSnapAfterDelay = false;
					mSpriteMomentum.deactivate();
					mScroller->tweenPosition(
						tweenDestination, mReturnAnimateTime, 0.0f, ci::EaseOutQuint(), [this]() { tweenComplete(); },
...
void ScrollArea::drawClient(const ci::mat4& transformMatrix, const ds::DrawParams& drawParams) {
	...
	forEachChild([](auto& spr) { spr.setBlendMode(ds::ui::FBO_IN); }, true);
