...
const std::string FadeFrag = "uniform sampler2D  tex0;\n"
...
const std::string FadeVert = "#version 150\n"
...
namespace ds { namespace ui {
	...
	ScrollArea::ScrollArea(ds::ui::SpriteEngine& engine, const float startWidth, const float startHeight,
	  ...
	  , mSkipBoundCheck(false) {
		...
		if (mScroller) {
			...
			mScroller->setProcessTouchCallback(
				[this](Sprite* bs, const ds::ui::TouchInfo& ti) { handleScrollTouch(bs, ti); });
	...
	void ScrollArea::checkBounds(const bool immediate) {
		...
		if (doTween) {
			if (immediate) {
				...
				callAfterDelay(
					[this, doTween, tweenDestination]() {
						mWillSnapAfterDelay = false;
						mSpriteMomentum.deactivate();
						mScroller->tweenPosition(
							tweenDestination, mReturnAnimateTime, 0.0f, ci::EaseOutQuint(),
							[this]() { tweenComplete(); }, [this]() { scrollerTweenUpdated(); });
						scrollerUpdated(ci::vec2(tweenDestination));
					},
					0.0f);
			}
	...
	void ScrollArea::drawClient(const ci::mat4& transformMatrix, const ds::DrawParams& drawParams) {
		...
		forEachChild([this](auto& spr) { spr.setBlendMode(ds::ui::FBO_IN); }, true);
		...
		ci::gl::drawSolidRect(
			ci::Rectf(0.0f, 0.0f, (float)sourceTexture->getWidth(), (float)sourceTexture->getHeight()));
	...
	void
	ScrollArea::setSnapToPositionCallback(const std::function<void(ScrollArea*, Sprite*, bool&, ci::vec3&)>& func) {
	...
	void ScrollArea::tweenScrollPercent(const float percenty) {
		...
		tweenNormalized(
			mReturnAnimateTime, 0.0f, ci::easeOutQuint, [this] { scrollerUpdated(ci::vec2(mScroller->getPosition())); },
	...
	void ScrollArea::scrollPage(const bool forwards, const bool animate) {
		...
		mSpriteMomentum.deactivate();
		if (animate) {
			mScroller->tweenPosition(tweenDestination, mReturnAnimateTime, 0.0f, ci::EaseInOutQuint(), nullptr,
