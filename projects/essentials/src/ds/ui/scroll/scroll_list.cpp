...
namespace ds { namespace ui {
	...
	ScrollList::ScrollList(ds::ui::SpriteEngine& engine, const bool vertical)
	  ...
	  , mMatrixPadding(0) {
		...
		if (mScrollArea) {
			...
			mScrollArea->setScrollerTouchedCallback(
				[this] { mLastUpdateTime = Poco::Timestamp().epochMicroseconds(); });
						...
						if (!mReserveItems.empty()) {
							...
							if (sprite) {
								sprite->setProcessTouchCallback(
									[this](ds::ui::Sprite* sp, const ds::ui::TouchInfo& ti) {
	...
	void ScrollList::setLayoutParams(const float startPositionX, const float startPositionY,
									 const float incremenetAmount, const bool fill_from_top) {
