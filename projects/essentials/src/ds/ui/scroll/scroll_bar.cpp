...
namespace ds { namespace ui {
	...
	ScrollBar::ScrollBar(ds::ui::SpriteEngine& engine, const bool vertical, const float uiWidth,
						 const float touchPadding, const bool autoHide)
	  : Sprite(engine)
	  , mVertical(vertical)
	  , mNub(nullptr)
	  , mBackground(nullptr)
	...
	void ScrollBar::linkScrollArea(ds::ui::ScrollArea* area) {
		...
		setScrollMoveCallback([this, area](const float scrollPercent) { area->setScrollPercent(scrollPercent); });
	}
