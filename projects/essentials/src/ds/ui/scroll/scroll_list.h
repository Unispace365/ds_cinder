 ...
 * This assumes that you can refer to your content by integers only, so you may have to keep a map in your super
 * class.
 *
 * NOTE: In perspective sprites, the list will fill up from the bottom. If you need to modify this, leave filling
 * from the bottom the default.
 */
...
class ScrollList : public ds::ui::Sprite {
  public:
	...
	/// \param fill_from_top Whether to align to the bottom of the scroll area or the top. For instance, if there's
	/// not enough items to fill the whole space, will start filling and align to the bottom if this param is false.
	void setLayoutParams(const float startPositionX, const float startPositionY, const float incremenetAmount,
						 const bool fill_from_top = true);
	...
	/// Makes a grid that goes left-to-right, then top-to-bottom with a set advance increment between the columns
	/// and rows
	...
	/// Run the layout of children, just in case you need to force the layout. Most changes induce this
	/// automatically.
	virtual void layout();
  ...
  protected:
	...
	//		- Set Data: clear out old data, and build a list of placeholders. These are just the data (product), a
	// position and a pointer to a sprite
	//		- layoutItems: place the placeholders on a virtual grid.
	//		- assignItems: figure out which placeholders should be onscreen and associate sprites with them. New
	// sprites can be created here if needeed. 						extra sprites are kept in mReserveI tems.
