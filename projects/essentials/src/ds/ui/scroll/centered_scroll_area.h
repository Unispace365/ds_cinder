...
namespace ds { namespace ui {
	...
	class CenteredScrollArea : public ScrollArea {
	  public:
		CenteredScrollArea(ds::ui::SpriteEngine& engine, const float startWidth, const float startHeight,
		...
		// It can be useful for temporarily moving an item to the center in a tween, and then following up with a
		// postFunc.
		void centerOnIndex(int index, float duration = 0.0f, float delay = 0.0f, const ci::EaseFn& ease = ci::easeNone,
						   const std::function<void()>& postFunc = nullptr);
		...
		void balanceOnIndex(int index, float duration = 0.0f, float delay = 0.0f, const ci::EaseFn& ease = ci::easeNone,
							const std::function<void()>& postFunc = nullptr);
