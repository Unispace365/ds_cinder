...
const std::string vertShader = "uniform mat4	ciModelViewProjection;\n"
							   "in vec4			ciPosition;\n"
							   "in vec4			ciColor;\n"
							   "out vec4			Color;\n"
							   "in vec2			ciTexCoord0;\n"
		...
		if (mFbo) {
			...
			if (shaderBase) {
				...
				if (!getPerspective()) {
					...
					ci::gl::drawSolidRect(ci::Rectf(0.0f, static_cast<float>(mFbo->getHeight()),
													static_cast<float>(mFbo->getWidth()), 0.0f));
				}
