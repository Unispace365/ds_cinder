...
class SmartLayout : public ds::ui::LayoutSprite {
  public:
	...
	bool hasSprite(std::string spriteName) {
		return mSpriteMap[spriteName] != nullptr;
	}

	ds::ui::Sprite* getSprite(std::string spriteName) {
		return mSpriteMap.at(spriteName);
	}
	template <typename T>
	T* getSprite(std::string spriteName) {
		return dynamic_cast<T*>(mSpriteMap.at(spriteName););
	}
	...
	void setSpriteTapFn(std::string spriteName,
