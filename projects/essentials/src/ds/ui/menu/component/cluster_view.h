...
namespace ds { namespace ui {
	...
	class ClusterView : public ds::ui::Sprite {
	  public:
		...
		/// animates this cluster off and prepares it for reuse. Called internally when there are no more fingers
	  ...
	  private:
		...
		// a cluster view is invalid if the current cluster has grown too big or moved too far from it's 4-finger start
		// point. A cluster deactivates when all associated fingers are removed from the wall.
		void activate();
		void invalidate();
