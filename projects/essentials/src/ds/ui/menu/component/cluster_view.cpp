...
namespace ds { namespace ui {
	...
	void ClusterView::startTappableMode(const ci::vec3& globalLocation, const float timeoutTime) {
		...
		if (mBackground) {
			...
			mBackground->setTapCallback([this](ds::ui::Sprite*, const ci::vec3& pos) { cancelTappableMode(); });
		}
	}
	...
	void ClusterView::updateCluster(const ds::ui::TouchInfo::Phase			  btp,
									const ds::ui::FiveFingerCluster::Cluster& cluster) {
		if (!mActive && cluster.mTouches.size() >= mMenuConfig.mClusterMinTouchPoints &&
	...
	bool ClusterView::validateCluster(const ds::ui::FiveFingerCluster::Cluster& cluster, const bool calcDist) {
		...
		if (cluster.mCurrentBoundingBox.getWidth() > sizeThreshold ||
			cluster.mCurrentBoundingBox.getHeight() > sizeThreshold) {
			return false;
		}
	...
	void ClusterView::activate() {
		...
		if (mBackground) {
			...
			mBackground->tweenScale(ci::vec3(bgScale, bgScale, 1.0f), mMenuConfig.mAnimationDuration, 0.0f,
									ci::easeInOutCubic, [this] { pulseBackground(true); });
	...
	void ClusterView::pulseBackground(const bool bigger) {
		...
		mBackground->tweenScale(ci::vec3(destScale, destScale, 1.0f), mMenuConfig.mAnimationDuration * 2.0f, 0.0f,
								ci::easeOutCubic, [this, bigger] { pulseBackground(!bigger); });
	}
	...
	void ClusterView::invalidate() {
		if (!mInvalid) {
			...
			if (mBackground) {
				mBackground->animStop();
				mBackground->tweenScale(ci::vec3(), mMenuConfig.mAnimationDuration, 0.0f, ci::easeInCubic,
										[this] { handleInvalidateComplete(); });
