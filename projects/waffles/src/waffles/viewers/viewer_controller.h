...
class ViewerController : public ds::ui::Sprite {
  public:
	...
	void setLayerBounds(int viewLayer, ci::Rectf bounds);

	/// Add a new viewer with the specified type and media
	void addViewer(ViewerCreationArgs& creationArgs, const float delay = 0.0f);
	...
	void animateViewerOff(BaseElement* viewer, const float delayey, const int style);
	...
	void gatherViewers(const ci::vec3& location);
	...
	std::vector<BaseElement*>& getViewers() { return mViewers; }

	std::vector<BaseElement*> getViewersOfType(const std::string& type);

	std::vector<BaseElement*> getViewersWithResourceId(const ds::Resource::Id& resourceId);

	// Makes the supplied viewer fullscreen and adds a black layer behind it
	void fullscreenViewer(BaseElement* viewer, const bool immediate, const bool showController = true);

	// Makes the supplied viewer not fullscreen and removes any associated black layers behind it
	void unfullscreenViewer(BaseElement* viewer, const bool immediate);
  ...
  protected:
	...
	void enforceViewerLimits(BaseElement* viewer);
	void gatherAviewer(BaseElement* viewer, const ci::vec3& pos);

	// responds true if there was a pdf, false if this was not handled
	bool advancePDF(const bool forwards);

	void startPresentation(ds::model::ContentModelRef newPresentation, const ci::vec3& startLocation,
						   const bool showController);
	void endPresentation();
	void advancePresentation(const bool forwards);
	void setPresentationSlide(int slideId);

	// Do not call directly, this should only be called from the app event
	void loadPresentationSlide(ds::model::ContentModelRef slideRef);
	void loadSlideBackground(ds::model::ContentModelRef slideRef);
	void loadSlideComposite(ds::model::ContentModelRef slideRef);
