...
class IPdf : public ds::ui::Sprite {
  public:
	...
	/** Returns the number of pages in this document. PDF's are 1-indexed, so the number returned from this function
	 * can be set as the last page */
	...
	virtual void setLinkClickedCallback(std::function<void(ds::pdf::PdfLinkInfo info)> func) {
		mLinkClickedCallback = func;
	};
...
class Pdf : public IPdf {
  public:
	...
	/// Callback when the page size changes. Highly recommend you listen for this. The sprite will change size
	...
	/** When the page renders, will show any links as enabled sprites and send clicked callbacks if they are tapped
	 */
	void showLinks() override;

	/** Hides any links that were previously shown from showLinks() */
	void hideLinks() override;

	/** When an internal link has been clicked. Requires links to be shown */
	void setLinkClickedCallback(std::function<void(ds::pdf::PdfLinkInfo info)> func) override {
		mLinkClickedCallback = func;
	};
	// END OF IPDF API

	/** Called when a page has finished drawing. This may also be called if the PDF is re-scaled and redraws again.
	 * Expect this to be called many times */
...
#ifdef _DEBUG
	virtual void writeState(std::ostream&, const size_t tab) const override;
#endif
