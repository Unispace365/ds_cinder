...
std::multimap<t_myVector2, t_myVector2> t_algoGil::pack(const std::vector<t_myVector2>& temp, const t_myVector2& size,
														bool& fine) {
	...
	while (!rects.empty()) {
		...
		for (auto iterRects = rects.begin(); iterRects != rects.end(); iterRects++) {
			for (auto iterFreeBoxes = freeBoxes.begin(); iterFreeBoxes != freeBoxes.end(); iterFreeBoxes++) {
				...
				// std::cout<<"The box "<<iterFreeBoxes->pos<<" with a size "<<iterFreeBoxes->size<<" is too small for
				// "<<*iterRects<<std::endl;
		...
		t_myBox tempbox(t_myVector2(minIterFreeBoxes->pos.x + minIterRects->x, minIterFreeBoxes->pos.y + 0),

						t_myVector2(minIterFreeBoxes->size.x - minIterRects->x, minIterFreeBoxes->size.y));


		t_myBox tempbox2(t_myVector2(minIterFreeBoxes->pos.x + 0, minIterFreeBoxes->pos.y + minIterRects->y),

						 t_myVector2(minIterRects->x, minIterFreeBoxes->size.y - minIterRects->y));
