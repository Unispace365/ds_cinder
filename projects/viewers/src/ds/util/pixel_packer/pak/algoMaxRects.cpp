...
std::multimap<t_myVector2, t_myVector2> t_algoMaxRects::pack(const std::vector<t_myVector2>& temp,
															 const t_myVector2& size, bool& fine) {
	...
	while (!rects.empty()) {
		...
		for (auto iterRects = rects.begin(); iterRects != rects.end(); iterRects++) {
			for (auto iterFreeBoxes = freeBoxes.begin(); iterFreeBoxes != freeBoxes.end(); iterFreeBoxes++) {
				...
				if (distance < 0) {
					// std::cout<<"The box "<<iterFreeBoxes->pos<<" with a size "<<iterFreeBoxes->size<<" is too small
					// for "<<*iterRects<<std::endl;
					continue;
				}

				if (distance < min) {
					// std::cout<<"The box "<<iterFreeBoxes->pos<<" with a size "<<iterFreeBoxes->size<<" is ok for
					// "<<*iterRects<<std::endl;
					min				 = distance;
		...
		freeBoxes.push_front(
			t_myBox(t_myVector2(minIterFreeBoxes->pos.x + minIterRects->x, minIterFreeBoxes->pos.y + 0),

					t_myVector2(minIterFreeBoxes->size.x - minIterRects->x, minIterFreeBoxes->size.y)));


		freeBoxes.push_front(
			t_myBox(t_myVector2(minIterFreeBoxes->pos.x + 0, minIterFreeBoxes->pos.y + minIterRects->y),

					t_myVector2(minIterFreeBoxes->size.x, minIterFreeBoxes->size.y - minIterRects->y)));
		...
		for (auto iterFreeBoxes = freeBoxes.begin(); iterFreeBoxes != freeBoxes.end();) {
			...
			t_myVector2 b3 =
				t_myVector2(iterFreeBoxes->pos.x + iterFreeBoxes->size.x, iterFreeBoxes->pos.y + iterFreeBoxes->size.y);
			t_myVector2 b4 = t_myVector2(iterFreeBoxes->pos.x, iterFreeBoxes->pos.y + iterFreeBoxes->size.y);

			if ((a1.x >= b3.x) || (a1.y >= b3.y) || (a3.x <= b1.x) || (a3.y <= b1.y)) {
				iterFreeBoxes++;
			...
			{
				freeBoxes.push_front(t_myBox(t_myVector2(iterFreeBoxes->pos.x, iterFreeBoxes->pos.y),

											 t_myVector2(a1.x - b1.x, iterFreeBoxes->size.y)));
			}

			if ((a3.x < b3.x) /*  && (a1.x < b3.x) */) // If there is a chance that line a1,a2 is in the shape b
			{
				freeBoxes.push_front(t_myBox(t_myVector2(a3.x, iterFreeBoxes->pos.y),

											 t_myVector2(b3.x - a3.x, iterFreeBoxes->size.y)));
			}
			...
			if ((a1.y > b1.y) /*  && (a1.x < b3.x) */) // If there is a chance that line a1,a2 is in the shape b
			{
				freeBoxes.push_front(t_myBox(t_myVector2(iterFreeBoxes->pos.x, iterFreeBoxes->pos.y),
											 ...
											 t_myVector2(iterFreeBoxes->size.x, a1.y - b1.y)));
			}

			if ((a3.y < b3.y) /*  && (a1.x < b3.x) */) // If there is a chance that line a1,a2 is in the shape b
			{
				freeBoxes.push_front(t_myBox(t_myVector2(iterFreeBoxes->pos.x, a3.y),

											 t_myVector2(iterFreeBoxes->size.x, b3.y - a3.y)));
			}
		...
		for (auto iterFreeBoxes = freeBoxes.begin(); iterFreeBoxes != freeBoxes.end(); iterFreeBoxes++) {
			...
			while (iterFreeBoxes2 != freeBoxes.end()) {
				...
				t_myVector2 b1 = iterFreeBoxes->pos;
				t_myVector2 b2 = t_myVector2(iterFreeBoxes->pos.x + iterFreeBoxes->size.x, iterFreeBoxes->pos.y);
				t_myVector2 b3 = t_myVector2(iterFreeBoxes->pos.x + iterFreeBoxes->size.x,
											 iterFreeBoxes->pos.y + iterFreeBoxes->size.y);
				t_myVector2 b4 = t_myVector2(iterFreeBoxes->pos.x, iterFreeBoxes->pos.y + iterFreeBoxes->size.y);
				...
				t_myVector2 c3 = t_myVector2(iterFreeBoxes2->pos.x + iterFreeBoxes2->size.x,
											 iterFreeBoxes2->pos.y + iterFreeBoxes2->size.y);
				...
				if (c1.x >= b1.x && c1.y >= b1.y && c3.x <= b3.x && c3.y <= b3.y) {
					// std::cout<<"Erasing a box "<<iterFreeBoxes->pos<<", "<<iterFreeBoxes->size<<" and
					// "<<iterFreeBoxes2->pos<<", "<<iterFreeBoxes2->size<<std::endl;
