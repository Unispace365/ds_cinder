...
#include "ds/ui/media/interface/oneline_pdf_interface.h"
...
namespace ds { namespace ui { namespace MediaInterfaceBuilder {

	static MediaInterfaceFactoryFunc builderFunc;

	MediaInterface* buildMediaInterface(ds::ui::SpriteEngine& engine, ds::ui::Sprite* mediaPlayer,
										ds::ui::Sprite* parentSprite, const ci::Color buttonColor,
	...
	MediaInterface* defaultBuildMediaInterface(ds::ui::SpriteEngine& engine, ds::ui::Sprite* mediaPlayer,
											   ds::ui::Sprite* parentSprite, const ci::Color buttonColor,
											   const ci::Color backgroundColor) {
		...
		auto buttonSize = engine.getViewersSettings().getFloat("ui:media_button:size", 0, 25.0f);
		auto barSize = engine.getViewersSettings().getFloat("ui:media_button_bar:size", 0, 50.0f);
		...
		if (vidPlayer) {
			ds::ui::VideoInterface* vi =
				new VideoInterface(engine, ci::vec2(1050.0f, barSize), buttonSize, buttonColor, backgroundColor);
		...
		if (webPlayer) {
			ds::ui::WebInterface* wi =
				new WebInterface(engine, ci::vec2(400.0f, barSize), buttonSize, buttonColor, backgroundColor);
		...
		if (ytPlayer) {
			ds::ui::YoutubeInterface* wi =
				new YoutubeInterface(engine, ci::vec2(400.0f, barSize), buttonSize, buttonColor, backgroundColor);
			parentSprite->addChildPtr(wi);
		...
		if (pdfPlayer) {
			ds::ui::PDFInterface* pi =
				new PDFInterface(engine, ci::vec2(400.0f, barSize), buttonSize, buttonColor, backgroundColor);
		...
		if (!outputMi) {
			DS_LOG_WARNING("This type of media player isn't supported in buildMediaInterface. Support it! Or pass in "
	...
	MediaInterface* framedBuildMediaInterface(ds::ui::SpriteEngine& engine, ds::ui::Sprite* mediaPlayer,
											   ds::ui::Sprite* parentSprite, const ci::Color buttonColor,
											   const ci::Color backgroundColor) {
		MediaInterface* outputMi = nullptr;
...
}}} // namespace ds::ui::MediaInterfaceBuilder
