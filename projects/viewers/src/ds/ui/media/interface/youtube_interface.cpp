...
YoutubeInterface::YoutubeInterface(ds::ui::SpriteEngine& eng, const ci::vec2& sizey, const float buttonHeight, const ci::Color buttonColor, const ci::Color backgroundColor)
	...
	, mWebLocked(false)
{
	...
	*/
	...
	mMinWidth = (
		padding * 4.0f +
		mVolumeControl->getScaleWidth()  + padding + 
...
void YoutubeInterface::onLayout() {
	...
	if (mScrubBar && mVolumeControl && mPlayButton) {
		...
		if (mForwardPageButton && mBackPageButton && mVolumeControl) {
			mForwardPageButton->setPosition(getWidth() / 2.0f + w / 2.0f - mVolumeControl->getWidth() - padding * 2.0f - mForwardPageButton->getScaleWidth(), h / 2.0f - mForwardPageButton->getScaleHeight() / 2.0f);
			mBackPageButton->setPosition(getWidth() / 2.0f + w / 2.0f - mVolumeControl->getWidth() - padding * 2.0f - mBackPageButton->getScaleWidth(), h / 2.0f - mBackPageButton->getScaleHeight() / 2.0f);
			spaceLeft -= mBackPageButton->getScaleWidth() + padding;
		}
		*/
