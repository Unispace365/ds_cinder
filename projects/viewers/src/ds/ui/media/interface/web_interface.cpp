...
namespace ds { namespace ui {
	...
	WebInterface::WebInterface(ds::ui::SpriteEngine& eng, const ci::vec2& sizey, const float buttonHeight,
	  ...
	  , mEventClient(mEngine) {
		...
		mMinWidth = (mKeyboardButton->getScaleWidth() + padding + mBackButton->getScaleWidth() + padding +
					 mForwardButton->getScaleWidth() + padding + mRefreshButton->getScaleWidth() + padding +
	...
	void WebInterface::startAuthCallback(const std::string& host, const std::string& realm) {
		...
		cancelButton->setTapCallback([this](ds::ui::Sprite*, const ci::vec3&) { cancelAuth(); });
	...
	void WebInterface::onLayout() {
		...
		if (mKeyboardButton && mBackButton && mForwardButton && mRefreshButton && mTouchToggle) {
			...
			float componentsWidth = (mKeyboardButton->getScaleWidth() + padding + mBackButton->getScaleWidth() +
									 padding + mForwardButton->getScaleWidth() + padding +
									 mRefreshButton->getScaleWidth() + padding + mTouchToggle->getScaleWidth());
