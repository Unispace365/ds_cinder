...
struct MediaViewerSettings {
	...
	/// Whether the interface should be hidden always (true by default)
	bool mCanDisplayInterface;

	/// If true, puts the interface below (vertically) the media, otherwise floats on top on the lower third (the
	/// default)
	...
	/// If true, centers and fits the content. If false, will expand the content so it fills the player (doesn't
	/// clip by default though)
	bool mLetterBox;
	...
	// Allow web sprites to retain native keyboard input even when the software keyboard is not on screen
	bool mWebNativeKeyboardOnly;
	...
	//--------------------YouTube Settings ------------------------------------------//
	// Note that some video and web settings are used for youtube as well (autoplay, volume, start touchable, native
	// touches)
	...
	/// If the video goes out of the current bounds of the window, mute the video automatically (or don't have that
	/// functionality)
