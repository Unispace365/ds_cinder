...
namespace ds { namespace web {
	...
	WebCefService::WebCefService(ds::Engine& e)
	  : ds::AutoUpdate(e, AutoUpdateType::SERVER | AutoUpdateType::CLIENT) {
		mEngine.registerSpriteImporter(
			"web", [this](ds::ui::SpriteEngine& engine) -> ds::ui::Sprite* { return new ds::ui::Web(mEngine); });

		mEngine.registerSpritePropertySetter(
			"web_url", [this](ds::ui::Sprite& bs, const std::string& theValue, const std::string& fileReferrer) {
		...
		try {
			...
			for (int i = 0; i < MAX_TRIES; i++) {
				if (CefPostTask(TID_UI, base::Bind(&shutdownCefCallbackFunc))) {
					...
					DS_LOG_WARNING("WebCefService: CefQuitMessageLoop message was not successfully sent.  Trying "
								   << (MAX_TRIES - i - 1) << " more times...");
	...
	void WebCefService::start() {
		...
		// I couldn't actually get this working. You're supposed to copy the manifest.json and widevine dll's from your
		// chrome directory But for whatever reason, Netflix and Hulu still complain about missing stuff. Version
		// mismatch? I dunno. Anyways, this is here.
		...
		*/
		...
		// CEF handles threading for the message loop (required for performance, otherwise slow apps can become
		// deadlocked)
		...
		*/
		...
		CefString(&settings.browser_subprocess_path).FromString(cef_dir + path);
	...
	void WebCefService::createBrowser(const std::string& startUrl, void* instancePtr,
									  std::function<void(int)> createdCallback, const bool isTransparent) {
		if (mCefSimpleApp) {
			try {
	...
	void WebCefService::sendMouseClick(const int browserId, const int x, const int y, const int bttn, const int state,
	...
	void WebCefService::sendMouseWheelEvent(const int browserId, const int x, const int y, const int xDelta,
											const int yDelta) {
		CefRefPtr<WebHandler> handler(WebHandler::GetInstance());
		if (handler) {
	...
	void WebCefService::sendTouchEvent(const int browserId, const int touchId, const int x, const int y,
									   const int phase) {
		CefRefPtr<WebHandler> handler(WebHandler::GetInstance());
		if (handler) {
	...
	void WebCefService::sendKeyEvent(const int browserId, const int state, int windows_key_code, char character,
	...
	void WebCefService::authCallbackContinue(const int browserId, const std::string& username,
