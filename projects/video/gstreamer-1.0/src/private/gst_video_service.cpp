...
GstVideoService::GstVideoService(ds::Engine& e)
  ...
  , mEngine(e) {
	...
	mEngine.registerSpritePropertySetter(
		"video_src", [](ds::ui::Sprite& theSprite, const std::string& theValue, const std::string& fileReferrer) {
	...
	mEngine.registerSpritePropertySetter(
		"stream_src", [](ds::ui::Sprite& theSprite, const std::string& theValue, const std::string& fileReferrer) {
			...
			if (streamTokens.size() < 3) {
				...
				if (pipeline.empty() || widdy < 1 || hiddy < 1) {
					DS_LOG_WARNING("Incorrect parameters for xml-loaded video stream. pipeline="
								   << pipeline << ", w=" << widdy << ", h=" << hiddy);
				} else {
	...
	mEngine.registerSpritePropertySetter<ds::ui::GstVideo>(
		"video_gl_mode", [](ds::ui::GstVideo& video, const std::string& theValue, const std::string& fileReferrer) {
	...
	mEngine.registerSpritePropertySetter<ds::ui::GstVideo>(
		"video_nvdecode", [](ds::ui::GstVideo& video, const std::string& theValue, const std::string& fileReferrer) {
...
void GstVideoService::start() {
	...
	DS_LOG_WARNING("No plugins added from the default directory, check your gstreamer install for version " <<
	gstVersion);
	}
	}
	*/
	...
	std::cout << "Plugin name: " << gst_plugin_get_name(plugin) << " version: " << gst_plugin_get_version(plugin) <<
	std::endl; gst_object_unref(plugin);
	}
	g_list_free(list);
	*/
	...
	if (ds::getLogger().hasVerboseLevel(3)) {
		...
		gst_debug_set_threshold_from_string("*:4", true); // 4 for gstreamer is the threshold for a TON of stuff
