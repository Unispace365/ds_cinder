...
namespace ds { namespace gstreamer {
	...
	bool EnvCheck::addGStreamerBinPath() {
		...
		// 1. Copy all the dll's / exe's from the gstreamer bin directory (c:/Program Files(x86)/gstreamer/x86/bin by
		// default (may be on a different drive)) to %APP%/dll
		//	  Copy all the dll's (you can ignore the folders) from the plugin directory
		//(c:\gstreamer\1.0\x86\lib\gstreamer-1.0) to "%APP%/dll/gst-plugins"
		//    These dll's can be included in your released packed so a local gstreamer install is not required

		// 2. Install the Gstreamer runtime msi on the target machine (
		// https://gstreamer.freedesktop.org/data/pkg/windows/1.8.2/gstreamer-1.0-x86-1.8.2.msi ) as of this writing
		...
		if (!addedLocalDlls && path_variable.find(gstreamer_bin_path) == std::string::npos &&
			ds::safeFileExistsCheck(gstreamer_bin_path, true)) {
