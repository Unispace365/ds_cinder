...
namespace ds { namespace ui {
	...
	namespace {
		const std::string& ERROR_TYPE_SZ() {
			static const std::string ANS("");
			return ANS;
		}
		const std::string& AUDIO_ONLY_TYPE_SZ() {
			static const std::string ANS("a");
			return ANS;
		}
		const std::string& VIDEO_ONLY_TYPE_SZ() {
			static const std::string ANS("v");
			return ANS;
		}
		const std::string& VIDEO_AND_AUDIO_TYPE_SZ() {
			static const std::string ANS("va");
			return ANS;
		}
	...
	bool VideoMetaCache::getValues(const std::string& videoPath, Type& outType, int& outWidth, int& outHeight,
								   double& outDuration, std::string& outColorSpace) {
	...
	void VideoMetaCache::setValues(Entry& entry) {
		...
		if ((entry.mType == VIDEO_ONLY_TYPE || entry.mType == VIDEO_AND_AUDIO_TYPE) &&
			(entry.mWidth < 1 || entry.mHeight < 1)) {
			DS_LOG_WARNING("Attempted to cache video with no size (path=" << entry.mPath << ", width=" << entry.mWidth
		...
		try {
			...
			if (ds::query::Client::query(db, buf.str(), ans) && !ans.rowsAreEmpty()) {
				buf.str("");
				buf << "UPDATE video_meta SET type='" << db_type_from_type(entry.mType) << "', width=" << entry.mWidth
					<< ", height=" << entry.mHeight << ", duration=" << entry.mDuration << ", colorspace='"
					<< entry.mColorSpace << "', videocodec='" << entry.mVideoCodec << "', audiocodec='"
					<< entry.mAudioCodec << "' WHERE path='" << entry.mPath << "'";
				ds::query::Client::queryWrite(db, buf.str(), ans);
				load();
				return;
			}
			buf.str("");
			buf << "INSERT INTO video_meta (type, path, width, height, duration, colorspace, videocodec, audiocodec) "
				   "values ('"
				<< db_type_from_type(entry.mType) << "', '" << entry.mPath << "', " << entry.mWidth << ", "
				<< entry.mHeight << ", " << entry.mDuration << ", '" << entry.mColorSpace << "', '" << entry.mVideoCodec
				<< "', '" << entry.mAudioCodec << "')";
			ds::query::Client::queryWrite(db, buf.str(), ans);
	...
	void VideoMetaCache::load() {
		...
		if (!f.exists()) {
			f.createFile();
			ds::query::Result r;
			ds::query::Client::queryWrite(
				db,
				"CREATE TABLE video_meta(id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT NOT NULL DEFAULT '', path "
				"TEXT NOT NULL DEFAULT '', width INT NOT NULL DEFAULT '0', height INT NOT NULL DEFAULT '0', duration "
				"DOUBLE NOT NULL DEFAULT '0', colorspace TEXT NOT NULL DEFAULT '', videocodec TEXT NOT NULL DEFAULT "
				"'', audiocodec TEXT NOT NULL DEFAULT '');",
				r);
		}

		ds::query::Result ans;
		ds::query::Client::query(
			db, "SELECT type, path, width, height, duration, colorspace, videocodec, audiocodec FROM video_meta", ans);
		ds::query::Result::RowIterator it(ans);
		while (it.hasValue()) {
			const std::string& game(it.getString(1));
			if (!game.empty())
				mEntries.push_back(Entry(game, type_from_db_type(it.getString(0)), it.getInt(2), it.getInt(3),
										 it.getFloat(4), it.getString(5), it.getString(6), it.getString(7)));
			++it;
		}
	}
	...
	bool VideoMetaCache::getVideoInfo(Entry& entry) {
		...
		if (!media_info.IsReady()) {
			...
			DS_LOG_ERROR(
				"VideoMetaCache::getVideoInfo() MediaInfo not loaded, does dll/MediaInfo.dll exist in the app folder?");
			return false;
		}
		...
		if (numAudio > 0) {
			entry.mAudioCodec =
				_MI_TO_STR(media_info.Get(MediaInfoDLL::Stream_Audio, 0, __T("Codec"), MediaInfoDLL::Info_Text));

			// std::cout << "Number of audio channels: " << _MI_TO_STR(media_info.Get(MediaInfoDLL::Stream_Audio, 0,
			// __T("Channels"), MediaInfoDLL::Info_Text)) << std::endl;
		}
		...
		if (numAudio > 0 && numVideo < 1) {
			...
			entry.mWidth  = 0;
			entry.mHeight = 0;
			if (!_MI_TO_VALUE(media_info.Get(MediaInfoDLL::Stream_Audio, 0, __T("Duration"), MediaInfoDLL::Info_Text),
							  entry.mDuration))
				return false;
			...
			if (!_MI_TO_VALUE(media_info.Get(MediaInfoDLL::Stream_Video, 0, __T("Width"), MediaInfoDLL::Info_Text),
							  entry.mWidth))
				return false;
			if (!_MI_TO_VALUE(media_info.Get(MediaInfoDLL::Stream_Video, 0, __T("Height"), MediaInfoDLL::Info_Text),
							  entry.mHeight))
				return false;

			// We don't check errors on these, cause they're not required by gstreamer to play a video, they're just
			// nice-to-have
			entry.mVideoCodec =
				_MI_TO_STR(media_info.Get(MediaInfoDLL::Stream_Video, 0, __T("Codec"), MediaInfoDLL::Info_Text));
			entry.mColorSpace =
				_MI_TO_STR(media_info.Get(MediaInfoDLL::Stream_Video, 0, __T("Colorimetry"), MediaInfoDLL::Info_Text));
			_MI_TO_VALUE(media_info.Get(MediaInfoDLL::Stream_Video, 0, __T("Duration"), MediaInfoDLL::Info_Text),
						 entry.mDuration);
		}
	...
	VideoMetaCache::Entry::Entry(const std::string& key, const Type t, const int width, const int height,
								 const double duration, const std::string& colorSpace, const std::string& videoCodec,
