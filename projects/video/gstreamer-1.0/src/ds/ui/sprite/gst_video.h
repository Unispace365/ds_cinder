...
class GstVideo : public Sprite {
  ...
  public:
	...
	// Transparent: retains an alpha channel through the whole pipeline, gstreamer handles the colorspace conversion
	// to RGBA (or BGRA) Solid: has no alpha channel, and gstreamer handles the colorspace conversion to RGB (or
	// BGR) ShaderTransform: has no alpha channel, and colorspace conversion is handled in a shader when drawing to
	// the screen, uses I420 YUV colorspace conversion only This value is assumed from the output of the videometa
	// cache Use these values if you're using the parseLaunch option
  ...
  public:
	...
	/// If all conditions are met, can greatly speed up playback, particularly on very large videos (4096 or 8192
	/// pixels depending on GPU) Must be set before loading the video
	...
	//		rtspsrc location=rtsp://192.168.1.37:5015/Stream1 ! application/x-rtp ! rtph264depay ! h264parse !
	// avdec_h264 ! queue ! videoconvert ! video/x-raw, format=(string)I420, width=1920, height=1080 ! appsink
	// name=appsink0 sync=true async=true qos=true
	// Or you can just specify the URI, and a playbin element will be auto-configured. Volume is supported for
	// audio, but not panning. Example uri:
	...
	/** Similar to startStream above, but this is not considered a live pipeline, and will only create a single
	   gstreamer element from the supplied pipeline. Assumes you really know what you're doing with gstreamer. This
	   assumes the source is part of the pipeline (so no filename from setResource or loadVideo is used) Not
	   currently setup for netsync (client/server) situations Note that width and height are enforced to be
	   multiples of 8 for I420 color space and multiples of 4 for the others
	*/
	void parseLaunch(const std::string& fullPipeline, const int videoWidth, const int videoHeight,
					 ...
					 const std::string& volumeElementName = "volume0", const double secondsDuration = -1);
	...
	/// Callback for generic gstreamer errors. The message comes from gstreamer itself, so it may not be appropriate
	...
	/// If you're in client-server mode and the server is in server-only mode (didn't actually load the video), and
	/// you have multiple clients that did load the video, you may get this message more than once
	...
	/// Play a single frame, then stop. Useful to show a thumbnail-like frame, or to keep a video in the background,
	/// but visible Optional: Supply the time in ms to display Optional: Supply a function called once that frame
	/// has been displayed (to unload the video, or animate or whatever) Optional: StopAfterFrame will stop the
	/// video and clear the pipeline, otherwise will pause and keep the pipeline open
	...
	/// This may interact badly with synchronization if you don't load the video on the ClientServer, so use with
	/// caution
	...
	/// The pan of a specific device. The most common case is to use setPan() above. This is only if you've
	/// setAudioDevices() and need to control a specific one
	...
	/// This is the "I'd really like to know the byte value of a pixel of the U channel before color space
	/// conversion" sorta thing
  ...
  protected:
	...
	virtual void writeClientAttributesTo(ds::DataBuffer&) override;
  ...
  private:
	...
	/// There's a server and a client, and the server didn't load the video but the client did (server-only mode on
	/// the server) The client's video got to the end, now the server neeeds to know about it. The client sets this
	/// flag to true, then the next writeClientAttributesTo() sends this flag to the server and resets it to false
