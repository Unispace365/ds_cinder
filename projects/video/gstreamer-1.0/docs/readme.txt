...
- Currently, dll's aren't checked into git, so you'll need to copy them yourself. When we go into production, we may want to check in the needed dll's
				...
				- NOTE NUMBER TWO: Setting the path is no longer required. The path is now set automatically in gst_video.cpp Init() object. In order for this to work correctly, some of the Gstreamer DLLs had to be delay loaded (check the property sheets for delay loaded DLLs).
