...
void linuxImplRegisterWindowFiledropHandler(ci::app::WindowRef cinderWindow) {
	...
	::glfwSetDropCallback((GLFWwindow*)sMainAppCinderWindow->getNative(),
						  [](GLFWwindow* window, int numPaths, const char** paths) {
	...
	mKeyManager.registerKey(
		...
		[this] {
			mEngineData.mSrcRect.x1 = mEngineData.mOriginalSrcRect.x1 + mEngineData.mOriginalSrcRect.getWidth() / 2.0f -
									  mEngineData.mDstRect.getWidth() / 2.0f;
			mEngineData.mSrcRect.x2 = mEngineData.mOriginalSrcRect.x1 + mEngineData.mOriginalSrcRect.getWidth() / 2.0f +
									  mEngineData.mDstRect.getWidth() / 2.0f;
			mEngineData.mSrcRect.y1 = mEngineData.mOriginalSrcRect.y1 +
									  mEngineData.mOriginalSrcRect.getHeight() / 2.0f -
									  mEngineData.mDstRect.getHeight() / 2.0f;
			mEngineData.mSrcRect.y2 = mEngineData.mOriginalSrcRect.y1 +
									  mEngineData.mOriginalSrcRect.getHeight() / 2.0f +
									  mEngineData.mDstRect.getHeight() / 2.0f;
			mEngine.markCameraDirty();
		},
...
static ds::Engine& new_engine(ds::App& app, ds::EngineSettings& settings, ds::EngineData& ed,
