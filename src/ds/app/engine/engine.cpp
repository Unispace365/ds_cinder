...
Engine::Engine(ds::App& app, ds::EngineSettings& settings, ds::EngineData& ed, const RootList& _roots,
  ...
  , mTouchEndedEvents(
		...
		"touchend")
  , mMouseBeginEvents(
		mTouchMutex, mLastTouchTime, [this](const MousePair& e) { handleMouseTouchBegin(e.first, e.second); },
		"mousebegin")
  , mMouseMovedEvents(
		mTouchMutex, mLastTouchTime, [this](const MousePair& e) { handleMouseTouchMoved(e.first, e.second); },
		"mousemoved")
  , mMouseEndedEvents(
		mTouchMutex, mLastTouchTime, [this](const MousePair& e) { handleMouseTouchEnded(e.first, e.second); },
		"mouseend")
  , mTuioObjectsBegin(
		mTouchMutex, mLastTouchTime, [&app](const TuioObject& e) { app.tuioObjectBegan(e); }, "tuiobegin")
  , mTuioObjectsMoved(
		mTouchMutex, mLastTouchTime, [&app](const TuioObject& e) { app.tuioObjectMoved(e); }, "tuiomoved")
  , mTuioObjectsEnded(
		mTouchMutex, mLastTouchTime, [&app](const TuioObject& e) { app.tuioObjectEnded(e); }, "tuioend")
  , mAutoHideMouse(true)
  , mHideMouse(false)
  , mUniqueColor(0, 0, 0)
  , mCachedWindowW(0)
...
void Engine::setupSrcDstRects() {
	...
	if (autoSizeMode == "all_span") {
		...
		if (!isFullscreen && newDstRect.getWidth() == mainDisplayRect.getWidth() &&
			newDstRect.getHeight() == mainDisplayRect.getHeight()) {
	...
	DS_LOG_INFO("Screen dst_rect is (" << mData.mDstRect.x1 << ", " << mData.mDstRect.y1 << ") - (" << mData.mDstRect.x2
									   << ", " << mData.mDstRect.y2 << ")");
}
...
void Engine::setupAutoSpan() {
	...
	if (autoSpan && ci::app::getWindow()) {
		...
		DS_LOG_INFO("Auto-spanning window, world size:" << mSettings.getSetting("world_dimensions", 0).mRawValue
														...
														<< " dst_rect:"
														<< mSettings.getSetting("dst_rect", 0).mRawValue);
	}
}
...
void Engine::setupResourceLocation() {
	...
	if (mData.mCmsURL.empty() || mData.mCmsURL == "DS_BASEURL") {
		...
		if (mData.mCmsURL.empty()) {
			DS_LOG_VERBOSE(1, "cms:url and DS_BASEURL are both not defined. Only a problem if this app relies on "
							  "Engine::getCmsURL()");
		} else {
...
void Engine::setupRoots() {
	...
	if (!isClient) {
		...
		for (auto it = roots.mRoots.begin(), end = roots.mRoots.end(); it != end; ++it) {
			...
			if (r.mType == r.kOrtho)
				root.reset(new OrthRoot(*this, r, r.mRootId));
			else if (r.mType == r.kPerspective)
				root.reset(new PerspRoot(*this, r, r.mRootId, r.mPersp));
			if (!root) {
...
void Engine::prepareSettings(ci::app::AppBase::Settings& settings) {
	...
	/// they're here too Sucks, but here it is
	...
	DS_LOG_INFO("Engine::prepareSettings: screenMode is " << screenMode << " and always on top "
...
void Engine::setup(ds::App& app) {
	...
	mTouchTranslator.setScale(mData.mSrcRect.getWidth() / ci::app::getWindowWidth(),
							  mData.mSrcRect.getHeight() / ci::app::getWindowHeight());
...
void Engine::createClientRoots(std::vector<RootList::Root> roots) {
	...
	for (auto it = roots.begin(), end = roots.end(); it != end; ++it) {
		...
		if (r.mType == r.kOrtho)
			...
			root.reset(new PerspRoot(*this, r, thisRootId, r.mPersp));
		if (!root) {
