...
void EngineStatsView::updateStats() {
	...
	if (mText) {
		...
		ss << "<span weight='bold'>Touch mode (t):</span> " << ds::ui::TouchMode::toString(mEngine.mTouchMode)
		   << std::endl;

		ss << "<span weight='bold'>Physical Memory:</span> "
		   << mEngine.getComputerInfo().getPhysicalMemoryUsedByProcess() << std::endl;
		ss << "<span weight='bold'>Virtual Memory:</span> " << mEngine.getComputerInfo().getVirtualMemoryUsedByProcess()
		   << std::endl;
		// ss << "<span weight='bold'>CPU:</span> " << mEngine.getComputerInfo().getPercentUsageCPU() << "%" <<
		// std::endl;
