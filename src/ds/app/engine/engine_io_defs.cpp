...
EngineIoInfo::EngineIoInfo(ds::Engine& engine) {
	if (engine.getEngineSettings().hasSetting("platform:guid")) {
		 ...
		 *	   That's not really true any more, and you can in fact run multiple clients on the same machine without
		 setting the guid. *	   However, there's some value to being able to specify an ID for each client, so this
		 setting is left here.
		 */
