...
class EngineCfg {
  public:
	...
	void loadText(const std::string& filename, Engine& engine);

  private:
	/// Make it easy for clients to access the engine settings.
	cfg::Settings&								   mEngineSettings;
	std::map<std::string, cfg::Settings>		   mSettings;
	std::unordered_map<std::string, ui::TextStyle> mTextStyles;
	ui::TextStyle								   mDefaultTextStyle;

	/// Empty settings for when some are missing. Here because we're getting
	/// a shutdown crash with this as statics.
	cfg::Settings mEmptySettings;
	cfg::Settings mEditEmptySettings;
};
