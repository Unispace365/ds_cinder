...
void NWQueryHandler::handleQuery() {
	...
	for (auto& it : rawUFNodesList) {
		...
		if (theType == "media" && it.getPropertyResource("media_res").empty()) {
			DS_LOG_WARNING("No media detected on raw node " << it.getId() << " " << it.getPropertyString("name"));
			continue;
		}
...
void NWQueryHandler::parseModelProperties(ds::model::ContentModelRef&			   it,
	...
	for (auto props : it.getChildren()) {
		...
		if (propName == "waffles_text_fields" || propName == "waffles_checkboxes" || propName == "waffles_color") {
			...
			auto theTMR = processResource(props.getPropertyResource("media_thumb_res"));

			// skip media fields where the resource is empty (deleted or not
			// downloaded yet)
			if (!theMR.empty() && !theTMR.empty()) {
				it.setProperty(appKey + "_media_res", props.getPropertyString("media_res"));
				it.setPropertyResource(appKey + "_media_res", theMR);
				it.setProperty(appKey + "_thumb_res", props.getPropertyString("media_thumb_res"));
				it.setPropertyResource(appKey + "_thumb_res", theTMR);
				if (appKey == "media") {
					it.setPropertyResource("media_res", theMR); // map a primary "media" to be "media_res"
				}
				if (props.getPropertyBool("floatable")) {
					it.setProperty(appKey + "_float_fullscreen", props.getPropertyString("float_fullscreen"));
					it.setProperty(appKey + "_float_autoplay", props.getPropertyString("float_autoplay"));
					it.setProperty(appKey + "_float_loop", props.getPropertyString("float_loop"));
					it.setProperty(appKey + "_float_volume", props.getPropertyString("float_volume"));
					it.setProperty(appKey + "_float_touch", props.getPropertyString("float_touch"));
					it.setProperty(appKey + "_float_page", props.getPropertyString("float_page"));
					it.setProperty(appKey + "_floatable", props.getPropertyString("floatable"));
					it.setProperty(appKey + "_hide_thumbnail", props.getPropertyString("hide_thumbnail"));
				}
			}

		} else if (propName == "waffles_dropdown_fields") {
			if (props.getPropertyBool("allow_multiple")) {
				it.addPropertyToList(appKey, props.getPropertyString("option_save_value"));
			} else {
				it.setProperty(appKey, props.getPropertyString("option_save_value"));
				it.setProperty(appKey + "_dropdown_label", props.getPropertyString("dropdown_label"));
			}

		} else if (propName == "waffles_selections") {
			if (appKey == "briefing_pinboard") {
				auto definitionId = props.getPropertyString("definition_id");
				mEngine.mContent.setProperty("briefing_pinboard_definition_id", definitionId);
			}
			...
			it.setProperty("composite_pos_x",
						   props.getPropertyString("pos_x")); // todo: handle multiple composites?
...
ds::Resource NWQueryHandler::processResource(ds::Resource input) {
	...
	if (input.getType() == ds::Resource::WEB_TYPE && input.getFileName().find("/youtube/") != std::string::npos) {
		...
		if (allPlatforms.hasChildren()) {
			...
			auto result = allPlatforms.getChild(0).getPropertyResource(input.getFileName());
			return result;
		}
