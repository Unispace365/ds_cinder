...
template <typename T>
void DataBuffer::add(const T& t) {
	mStream.write(reinterpret_cast<const char*>(&t), sizeof(t));
}

template <typename T>
T DataBuffer::read() {
	T t;
	mStream.read(reinterpret_cast<char*>(&t), sizeof(t));
	return t;
}

template <typename T>
void DataBuffer::rewindRead() {
	unsigned currentPosition = mStream.getReadPosition();

	if (sizeof(T) > currentPosition) return;

	mStream.rewindRead(sizeof(T));
}

template <typename T>
void DataBuffer::rewindAdd() {
