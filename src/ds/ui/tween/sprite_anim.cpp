...
namespace ds { namespace ui {
	...
	const SpriteAnim<float>& SpriteAnimatable::ANIM_OPACITY() {
		static ds::ui::SpriteAnim<float> ANIM([](ds::ui::Sprite& s) -> ci::Anim<float>& { return s.mAnimOpacity; },
	...
	const SpriteAnim<ci::vec3>& SpriteAnimatable::ANIM_SCALE() {
		static ds::ui::SpriteAnim<ci::vec3> ANIM([](ds::ui::Sprite& s) -> ci::Anim<ci::vec3>& { return s.mAnimScale; },
	...
	const SpriteAnim<float>& SpriteAnimatable::ANIM_NORMALIZED() {
		static ds::ui::SpriteAnim<float> ANIM(
			[](ds::ui::Sprite& s) -> ci::Anim<float>& {
				s.mNormalizedTweenValue = 0.0f;
				return s.mAnimNormalized;
			},
	...
	const bool SpriteAnimatable::animationRunning() {
		return getOpacityTweenIsRunning() || getPositionTweenIsRunning() || getScaleTweenIsRunning() ||
			   getSizeTweenIsRunning() || getRotationTweenIsRunning() || getColorTweenIsRunning() ||
	...
	void SpriteAnimatable::completeAllTweens(const bool callFinishFunctions, const bool recursive) {
		...
		if (mAnimScriptCueRef) {
			if (callFinishFunctions) {
				mAnimScriptCueRef->getFn()();
			}
	...
	float SpriteAnimatable::tweenAnimateOn(const bool recursive, const float delay, const float deltaDelay,
										   const std::function<void(void)>& finishFn) {
		if (mAnimScriptCueRef) mAnimScriptCueRef->removeSelf();
	...
	float SpriteAnimatable::tweenAnimateOff(const bool recursive, const float delay, const float deltaDelay,
											const std::function<void(void)>& finishFn) {
		if (mAnimScriptCueRef) mAnimScriptCueRef->removeSelf();
	...
	float SpriteAnimatable::runReversibleAnimationScript(const std::string& animScript, const float addedDelay,
		...
		for (auto it = animationCommands.begin(); it != animationCommands.end(); ++it) {
			...
			if (animType == "center") {
				...
				if (ci::vec2(currentCenter) == ci::vec2(dest)) break;

				mOwner.setCenter(dest.x, dest.y);
				mOwner.setPosition((dest.x - currentCenter.x) * mOwner.getScaleWidth() + currentPos.x,
								   (dest.y - currentCenter.y) * mOwner.getScaleHeight() + currentPos.y);
	...
	void SpriteAnimatable::runMultiAnimationScripts(const std::vector<std::string> animScripts, const float gapTime,
		...
		for (size_t i = 0; i < animScripts.size(); i++) {
			mMultiDelayedCallCueRefs.push_back(t.add(
				[this, delay, animScripts, i]() { runAnimationScript(animScripts[i]); }, t.getCurrentTime() + delay));
			delay += (durationList[i] + delayList[i] + gap);
		}
	}

	void SpriteAnimatable::parseMultiScripts(const std::vector<std::string> animScripts, std::vector<float>& durations,
	...
	ci::EaseFn SpriteAnimatable::getEasingByString(const std::string& inString) {
		...
		if (inString == "inQuad") {
			return ci::easeInQuad;
		}
		if (inString == "outQuad") {
			return ci::easeOutQuad;
		}
		if (inString == "inOutQuad") {
			return ci::easeInOutQuad;
		}
		if (inString == "inCubic") {
			return ci::easeInCubic;
		}
		if (inString == "outCubic") {
			return ci::easeOutCubic;
		}
		if (inString == "inOutCubic") {
			return ci::easeInOutCubic;
		}
		if (inString == "inQuart") {
			return ci::easeInQuart;
		}
		if (inString == "outQuart") {
			return ci::easeOutQuart;
		}
		if (inString == "inOutQuart") {
			return ci::easeInOutQuart;
		}
		if (inString == "inQuint") {
			return ci::easeInQuint;
		}
		if (inString == "outQuint") {
			return ci::easeOutQuint;
		}
		if (inString == "inOutQuint") {
			return ci::easeInOutQuint;
		}
		if (inString == "inSine") {
			return ci::easeInSine;
		}
		if (inString == "outSine") {
			return ci::easeOutSine;
		}
		if (inString == "inOutSine") {
			return ci::easeInOutSine;
		}
		if (inString == "inExpo") {
			return ci::easeInExpo;
		}
		if (inString == "outExpo") {
			return ci::easeOutExpo;
		}
		if (inString == "inOutExpo") {
			return ci::easeInOutExpo;
		}
		if (inString == "inCirc") {
			return ci::easeInCirc;
		}
		if (inString == "outCirc") {
			return ci::easeOutCirc;
		}
		if (inString == "inOutCirc") {
			return ci::easeInOutCirc;
		}
		if (inString == "inBounce") {
			return ci::EaseInBounce();
		}
		if (inString == "outBounce") {
			return ci::EaseOutBounce();
		}
		if (inString == "inOutBounce") {
			return ci::EaseInOutBounce();
		}
		if (inString == "inBack") {
			return ci::EaseInBack();
		}
		if (inString == "outBack") {
			return ci::EaseOutBack();
		}
		if (inString == "inOutBack") {
			return ci::EaseInOutBack();
		}
		if (inString == "inAtan") {
			return ci::EaseInAtan();
		}
		if (inString == "outAtan") {
			return ci::EaseOutAtan();
		}
		if (inString == "inOutAtan") {
			return ci::EaseInOutAtan();
		} else {
			return ci::easeNone;
		}
	}
