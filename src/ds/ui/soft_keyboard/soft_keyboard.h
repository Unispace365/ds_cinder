#pragma once
#ifndef DS_UI_SOFT_KEYBOARD_SOFT_KEYBOARD
#define DS_UI_SOFT_KEYBOARD_SOFT_KEYBOARD
...
namespace ds { namespace ui {
	...
	/**  Soft Keyboard
	 *		Owns the keyboard buttons, manages touch functions, and tracks the entered text (adds on most keys, subtracts
	 *on delete)
	 */
	class SoftKeyboard : public ds::ui::Sprite {
	  public:
		...
		/// Set a callback for every time a key is pressed by the user. KeyType is set in SoftKeyboardButton (letter,
		/// number, back, space, delete, etc)
		void setKeyPressFunction(
			const std::function<void(const std::wstring& character, const SoftKeyboardDefs::KeyType keyType)>&);
