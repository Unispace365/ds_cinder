...
namespace ds { namespace ui {
	...
	EntryField::EntryField(ds::ui::SpriteEngine& engine, EntryFieldSettings& settings)
	  ...
	  , mCursorIndex(0) {
		...
		setProcessTouchCallback([this](ds::ui::Sprite* bs, const ds::ui::TouchInfo& ti) { handleTouchInput(bs, ti); });
	...
	void EntryField::keyPressed(const std::wstring& keyCharacter, const ds::ui::SoftKeyboardDefs::KeyType keyType) {
		...
		if (keyType == SoftKeyboardDefs::kArrow) {
			...
			} else if (keyType == SoftKeyboardDefs::kShift || keyType == SoftKeyboardDefs::kFwdDelete) {
				// nothin!
			} else {
				mCursorIndex++;
			}
	...
	void EntryField::keyPressed(ci::app::KeyEvent& keyEvent) {
		...
		if (keyEvent.getCode() == ci::app::KeyEvent::KEY_BACKSPACE ||
			keyEvent.getCode() == ci::app::KeyEvent::KEY_DELETE) {
			...
			// TODO: handle up / down keys for lines and page up / down for pages
				   ...
				   keyEvent.getCode() == ci::app::KeyEvent::KEY_DOWN) {
			setCursorIndex(mCursorIndex + 1);
		} else if (keyEvent.getCode() == ci::app::KeyEvent::KEY_LEFT ||
				   keyEvent.getCode() == ci::app::KeyEvent::KEY_UP) {
			setCursorIndex(mCursorIndex - 1);
		} else if (keyEvent.getCode() == ci::app::KeyEvent::KEY_RSHIFT ||
				   keyEvent.getCode() == ci::app::KeyEvent::KEY_LSHIFT) {
			// we're just gonna ignore these guys
		} else if (keyEvent.getCode() == ci::app::KeyEvent::KEY_HOME ||
				   keyEvent.getCode() == ci::app::KeyEvent::KEY_PAGEUP) {
			setCursorIndex(0);
		} else if (keyEvent.getCode() == ci::app::KeyEvent::KEY_END ||
				   keyEvent.getCode() == ci::app::KeyEvent::KEY_PAGEDOWN) {
	...
	void EntryField::unfocus() {
		...
		if (mCursor) {
			mCursor->tweenOpacity(0.0f, mEntryFieldSettings.mAnimationRate, 0.0f, ci::easeNone,
								  [this] { mCursor->hide(); });
		}
	}
		...
		mCursor->tweenOpacity(1.0f, mEntryFieldSettings.mAnimationRate, 0.0f, ci::easeNone, [this] {
			mCursor->tweenOpacity(1.0f, mEntryFieldSettings.mBlinkRate, 0.0f, ci::easeNone, [this] {
				mCursor->tweenOpacity(0.0f, mEntryFieldSettings.mAnimationRate, 0.0f, ci::easeNone, [this] {
					mCursor->tweenOpacity(0.0f, mEntryFieldSettings.mBlinkRate, 0.0f, ci::easeNone,
										  [this] { blinkCursor(); });
				});
			});
		});
