...
namespace ds { namespace ui {
	SoftKeyboardButton::SoftKeyboardButton(ds::ui::SpriteEngine& engine, const std::wstring& characterlow,
										   const std::wstring& characterUp, const SoftKeyboardDefs::KeyType keyType,
	...
	const std::wstring& SoftKeyboardButton::getCharacter() {
		if (mUpper)
			return mCharacterUpper;
		else
			return mCharacterLower;
	}
	...
	void SoftKeyboardButton::setSoftKeyboardSettings(SoftKeyboardSettings& softKeySettings) {
		...
		if (softKeySettings.mGraphicKeys) {
			...
			if (mKeyType == SoftKeyboardDefs::kNumber || mKeyType == SoftKeyboardDefs::kFunction ||
				mKeyType == SoftKeyboardDefs::kArrow || mKeyType == SoftKeyboardDefs::kEscape) {
			...
			} else if (mKeyType == SoftKeyboardDefs::kTab || mKeyType == SoftKeyboardDefs::kDotCom ||
					   mKeyType == SoftKeyboardDefs::kSpecial) {
			...
			setSize(keySize.x + softKeySettings.mKeyTouchPadding * 2.0f,
					keySize.y + softKeySettings.mKeyTouchPadding * 2.0f);
			...
			setSize(floorf(mDownImg->getWidth() + softKeySettings.mKeyTouchPadding * 2.0f),
					floorf(mDownImg->getHeight() + softKeySettings.mKeyTouchPadding * 2.0f));
		}
		...
		if (mUpImg && mDownImg) {
			if (mPressed) {
				...
				if (mAnimDuration <= 0.0f) {
					...
					mDownImg->tweenOpacity(0.0f, mAnimDuration, 0.0f, ci::EaseInCubic(),
										   [this]() { mDownImg->hide(); });
				}
			}
