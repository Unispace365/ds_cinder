...
class ButtonBehaviour {
  public:
	...
	void setToScalePixel(const float pixels, const float pressedAnimDuration, const std::function<void(void)>& clickFn);

	typedef std::function<bool(const ci::vec3&)> TouchInsideCheckFunction;
  ...
  private:
	...
	std::function<void(const ds::ui::TouchInfo&)> mOnDownFn;
	...
	std::unordered_map<int, ci::vec3> mFinger;
	bool							  mIsSetToScale;
};
