...
namespace {
	void anim_scale_to(ds::ui::Sprite& s, const float scale, const float duration) {
		s.mAnimScale.stop();
		s.getEngine().getTweenline().apply(s, s.ANIM_SCALE(), ci::vec3(scale, scale, 1.0f), duration,
										   ci::easeInOutQuad);
	}

} // namespace

void ButtonBehaviour::setToScale(const float scale, const float dur, const std::function<void(void)>& clickFn) {
	mIsSetToScale = true;
	setOnDownFn([this, scale, dur](const ds::ui::TouchInfo&) { anim_scale_to(this->mOwner, scale, dur); });
	setOnUpFn([this, dur]() { anim_scale_to(this->mOwner, 1.0f, dur); });
	setOnEnterFn([this, scale, dur]() { anim_scale_to(this->mOwner, scale, dur); });
	setOnExitFn([this, dur]() { anim_scale_to(this->mOwner, 1.0f, dur); });
