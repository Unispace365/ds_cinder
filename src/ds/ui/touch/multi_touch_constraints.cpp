...
namespace ds { namespace ui {
	...
	const ds::BitMask parseMultitouchMode(const std::string& s) {
		...
		else if (boost::iequals(s, "pos"))
			return MULTITOUCH_CAN_POSITION;
		else if (boost::iequals(s, "all"))
			return MULTITOUCH_NO_CONSTRAINTS;
		else if (boost::iequals(s, "scale"))
			return MULTITOUCH_CAN_SCALE;
		else if (boost::iequals(s, "pos_x"))
			return MULTITOUCH_CAN_POSITION_X;
		else if (boost::iequals(s, "pos_y"))
			return MULTITOUCH_CAN_POSITION_Y;
		else if (boost::iequals(s, "pos_scale"))
			return MULTITOUCH_CAN_POSITION | MULTITOUCH_CAN_SCALE;
		else if (boost::iequals(s, "pos_rotate"))
			return MULTITOUCH_CAN_POSITION | MULTITOUCH_CAN_ROTATE;
		else if (boost::iequals(s, "rotate"))
			return MULTITOUCH_CAN_ROTATE;
		return MULTITOUCH_INFO_ONLY;
	}
