...
namespace ds { namespace ui {
	...
	void TuioInput::registerEvents() {
		...
		const auto transformCursor = [this, window](const ci::tuio::Cursor2d& tuioCursor) {
			...
			const ci::app::TouchEvent::Touch convertedTouch(pos, prevPos, tuioCursor.getSessionId() + mFingerIdOffset,
															touch.getTime(), (void*)touch.getNative());
		...
		mTuioReceiver->setAddedFn<ci::tuio::Cursor2d>([=](const auto& tuioCursor) {
			...
			if (mAllowedRect.calcArea() > 0.0f && !mAllowedRect.contains(pos)) {
				DS_LOG_VERBOSE(1, "TuioInput: discarding touch point at "
									  << pos << " because it doesn't fit in the filter rect " << mAllowedRect);
			} else {
				mEngine.injectTouchesBegin(transformed);
			}
		});
		mTuioReceiver->setUpdatedFn<ci::tuio::Cursor2d>(
			[=](const auto& tuioCursor) { mEngine.injectTouchesMoved(transformCursor(tuioCursor)); });
		mTuioReceiver->setRemovedFn<ci::tuio::Cursor2d>(
			[=](const auto& tuioCursor) { mEngine.injectTouchesEnded(transformCursor(tuioCursor)); });
	...
	void TuioInput::stop() {
		if (mStartListeningCallback) mEngine.cancelTimedCallback(mStartListeningCallback);
	...
	void TuioInput::start(const bool _registerEvents, const int port) {
		...
		if (_registerEvents) {
			...
			// TuioInputs that are not the main input have their own transformations, and setup their own handlers to
			// inject touches
			registerEvents();
		}
	...
	void TuioInput::startListening() {
		...
		try {
			DS_LOG_INFO("Starting TuioInput at port "
						<< mUdpPort << " with offset of " << ci::vec2(pos) << ", scale of " << ci::vec2(scale)
						<< ", and a rotation of " << glm::degrees(glm::roll(rot)) << " degrees.  finger Id offset of "
						<< mFingerIdOffset << " filter rect of " << mAllowedRect);
			mOscReceiver->bind();
		...
		mOscReceiver->listen([](asio::error_code ec, asio::ip::udp::endpoint ep) -> bool {
			if (ec) {
				DS_LOG_VERBOSE(
					1,
					"TuioInput Warning: Osc Receiver had error, most likely from resetting a listening UDP connection: "
						<< ec.message() << " Error Value: " << ec.value());
				return false;
			} else
				return true;
		});
