...
namespace ds { namespace ui {
			...
			if (chillin->mLayoutUserType == kFixedSize) {
				if (chillin->mLayoutSize.x > 0.0f && chillin->mLayoutSize.y > 0.0f) {
					...
					if (tp) {
					   ...
					   chillin->mLayoutUserType == kFillSize) {
	...
	void LayoutSprite::runFlowLayout(const bool vertical, const bool wrap /* = false*/) {
			...
			if (chillin->mLayoutUserType == kFillSize) {
				...
				if (chillin->mLayoutUserType == kStretchSize) {
					...
					if (chillin->mLayoutUserType == kFixedSize) {
						...
						if (chillin->mLayoutSize.x > 0.0f && chillin->mLayoutSize.y > 0.0f) {
							if (tp) {
								...
								fitInside(chillin,
										  ci::Rectf(0.0f, 0.0f, chillin->mLayoutSize.x, chillin->mLayoutSize.y), true);
							} else {
					...
					if (vertical) {
						...
						maxWidth = std::fmaxf(maxWidth,
											  (chillin->mLayoutLPad + chillin->getScaleWidth() + chillin->mLayoutRPad));
					} else {
						totalSize += chillin->mLayoutLPad + chillin->getScaleWidth() + chillin->mLayoutRPad;
						maxHeight = std::fmaxf(
							maxHeight, (chillin->mLayoutTPad + chillin->getScaleHeight() + chillin->mLayoutBPad));
					}
				}
			}
		}
		...
		// now that we know the offset and per stretch size, go through the children again, set position for all fixed,
		// flex, and stretch children and set the size of any stretch children
		for (auto chillin : mChildren) {
			...
			if (chillin->mLayoutUserType == kStretchSize) {
				...
				const float stretchW =
					(vertical ? layoutWidth : perStretch) - chillin->mLayoutLPad - chillin->mLayoutRPad;
				const float stretchH =
					(vertical ? perStretch : layoutHeight) - chillin->mLayoutTPad - chillin->mLayoutBPad;
			...
			if (wrap) {
				if (!vertical) {
					...
					if (offset + widdy > getWidth() && widdy < getWidth()) {
						...
						biggest =
							std::max(biggest, chillin->mLayoutTPad + chillin->getScaleHeight() + chillin->mLayoutBPad);
					}
				} else {
					...
					if (offset + hiddy > getHeight() && hiddy < getHeight()) {
						...
						biggest =
							std::max(biggest, chillin->mLayoutLPad + chillin->getScaleWidth() + chillin->mLayoutRPad);
					}
				}
			}
			...
			ci::vec2 childCenter(chillin->getCenter().x * chillin->getScaleWidth(),
								 chillin->getCenter().y * chillin->getScaleHeight());
			ci::vec2 totalOffset = ci::vec2(chillin->mLayoutFudge) + childCenter;
			if (wrap) {
				//	if(vertical)
				totalOffset += (vertical) ? ci::vec2(wrapOffset, 0.f) : ci::vec2(0.f, wrapOffset);
			}
		...
		if (hasFills) {
			for (auto chillin : mChildren) {
				...
				if (chillin->mLayoutUserType == kFillSize) {
					...
					// It's possible, after all this, that the child still might not have the full size of (fixedW,
					// fixedH). For example, images will be resized within their aspect, so they'll possibly be off.
					// Compensate for this by centering the child within the area defined by the padding, and respecting
					// the center and fudge factors.

					ci::vec2 centerOffset((fixedW - chillin->getScaleWidth()) * 0.5f,
										  (fixedH - chillin->getScaleHeight()) * 0.5f);
					ci::vec2 childCenter(chillin->getCenter().x * chillin->getScaleWidth(),
										 chillin->getCenter().y * chillin->getScaleHeight());
	...
	void LayoutSprite::runFlexLayout(bool calculate) {
		...
		std::function<void(std::vector<ds::ui::Sprite*>)> setupAutosizeChildren =
			[this, &setupAutosizeChildren](std::vector<ds::ui::Sprite*> children) {
		...
		std::function<void(std::vector<ds::ui::Sprite*>)> updateChildren =
			[this, &updateChildren](std::vector<ds::ui::Sprite*> children) {
				for (auto chillin : children) {
	...
	std::string LayoutSprite::getLayoutSizeModeString(const int sizeMode) {
		...
		if (sizeMode == ds::ui::LayoutSprite::kFlexSize)
			sizeString = "flex";
		else if (sizeMode == ds::ui::LayoutSprite::kStretchSize)
			sizeString = "stretch";
		else if (sizeMode == ds::ui::LayoutSprite::kFillSize)
			sizeString = "fill";
		return sizeString;
	}
	...
	std::string LayoutSprite::getLayoutVAlignString(const int vAlign) {
		...
		if (vAlign == ds::ui::LayoutSprite::kMiddle)
			sizeString = "middle";
		else if (vAlign == ds::ui::LayoutSprite::kBottom)
			sizeString = "bottom";
		return sizeString;
	}
	...
	std::string LayoutSprite::getLayoutHAlignString(const int vAlign) {
		...
		if (vAlign == ds::ui::LayoutSprite::kCenter)
			sizeString = "center";
		else if (vAlign == ds::ui::LayoutSprite::kRight)
			sizeString = "right";
		return sizeString;
	}
	...
	std::string LayoutSprite::getLayoutTypeString(const ds::ui::LayoutSprite::LayoutType& propertyValue) {
		...
		if (propertyValue == ds::ui::LayoutSprite::kLayoutVFlow)
			...
			sizeString = "horiz";
		else if (propertyValue == ds::ui::LayoutSprite::kLayoutVWrap)
			sizeString = "vert_wrap";
		else if (propertyValue == ds::ui::LayoutSprite::kLayoutHWrap)
			sizeString = "horiz_wrap";
