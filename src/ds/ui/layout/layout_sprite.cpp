		...
		if (chillin->mLayoutUserType == kFillSize) {
			...
			if (chillin->mLayoutUserType == kStretchSize) {
				...
				if (chillin->mLayoutUserType == kFixedSize) {
					...
					if (chillin->mLayoutSize.x > 0.0f && chillin->mLayoutSize.y > 0.0f) {
						if (tp) {
							...
							fitInside(chillin, ci::Rectf(0.0f, 0.0f, chillin->mLayoutSize.x, chillin->mLayoutSize.y),
									  true);
						} else {
				...
				if (vertical) {
					...
					maxWidth =
						std::fmaxf(maxWidth, (chillin->mLayoutLPad + chillin->getScaleWidth() + chillin->mLayoutRPad));
				} else {
					totalSize += chillin->mLayoutLPad + chillin->getScaleWidth() + chillin->mLayoutRPad;
					maxHeight = std::fmaxf(maxHeight,
										   (chillin->mLayoutTPad + chillin->getScaleHeight() + chillin->mLayoutBPad));
				}
			}
		}
	}
