...
namespace ds::ui {
	...
	class LayoutSprite : public ds::ui::Sprite {
	  public:
		...
		/// FlexSize = Sprite is adjusted to fit the layout. For example, in a V layout, the text is resized to the
		/// width, and the height is calculated StretchSize = Sprite will fill to the rest of the available space.
		/// Multiple Stretch sprites will evenly split the remainder FillSize = Sprite isn't used in V/H layout
		/// calculations, but is sized to the whole layout (minus padding). Great for backgrounds of flex size layouts.
		...
		/// AspectFill = fixed aspect will always try to fill the target rect. This may result in overflow, so you may
		/// want to clip AspectLetterbox = fixed aspect will letterbox the sprite.
		...
		/// Generally only has an effect for sizeType = kFixedSize, kStretchSize or fill; layoutType = kLayoutVFlow or
		/// kLayoutHFlow; and there are no stretch children
		...
		/**determines how the sprite adjusts to it's children.
			...
			2. width: Adjusts the width of this sprite to it's children (for vertical, the widest child, for horiz, the
		   total height of the children)
			3. height: Adjusts the height of this sprite to it's children (for vertical, the total height of the
		   children, for horiz, the tallest child)
		...
		/// If a child is hidden (hide() not setTransparent(false)) will be completely ignored in layouts. Default =
		/// false
