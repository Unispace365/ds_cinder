...
void DashedLine::rebuildLine() {
	...
	auto posVbo = ci::gl::Vbo::create(GL_ARRAY_BUFFER, positions.size() * sizeof(ci::vec3), positions.data(), GL_DYNAMIC_DRAW);
	auto scaleVbo = ci::gl::Vbo::create(GL_ARRAY_BUFFER, scales.size() * sizeof(ci::vec3), scales.data(), GL_DYNAMIC_DRAW);
