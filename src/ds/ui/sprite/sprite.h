...
namespace ui {
	...
	class Sprite : public SpriteAnimatable {
	  public:
		...
		/** Sprite creation convenience for this basic sprite type, throws on failure.
			\param engine The SpriteEngine for your app.
			\param parent An optional parent for the new sprite to be added to as a child.		*/
		static Sprite& makeSprite(SpriteEngine& engine, Sprite* parent = nullptr) {
			return makeAlloc<Sprite>([&engine]() -> Sprite* { return new Sprite(engine); }, parent);
		}
		...
		/** Draw function for when this app is set to be a client.
			In most cases, you'll want to override drawLocalClient() to do custom drawing, as this function handles
		   drawing for children as well. \param transformMatrix The transform matrix of the parent. \param drawParams
		   Parameters for drawing, such as the opacity of the parent.		*/
		...
		/** Returns the unique id for this Sprite. The SpriteEngine will automatically generate an id for the sprite
		   when constructed. We recommend you use references or pointers to keep track of sprites, rather than looking
		   up by Id. \return Unique sprite id.		*/
		...
		/** Sets the width and height of the Sprite.
		   ...
		   Image and Text. \param size2d The size to set in the form of ci::vec2f(width, height).		*/
		...
		/** Sets the width and height of the Sprite.
		   ...
		   not call this function yourself. \param width The new width of the Sprite. \param height The new height of
		   the Sprite.		*/
		...
		/** Sets the width, height, and depth of the Sprite.
			This does not affect the scale of the Sprite.
			Many subclasses set the size of the Sprite themselves, such as Image and Text, and in those cases you should
		   not call this function yourself. This is identical to setSize(), except it also sets the depth. \param width
		   The new width of the Sprite. \param height The new height of the Sprite. \param depth The new depth of the
		   Sprite.		*/
		...
		/** Answer the preferred size for this object.
			...
			\return The 3d vector of the size this sprite should be. Override this function and return a meaningful
		   value.		 */
		...
		/** The "middle" of the Sprite, NOT related to setCenter and getCenter (anchor), in this Sprite's co-ordinate
		   space. Effectively \code ci::vec3(floorf(mWidth/2.0f), floorf(mHeight/2.0f), mPosition.z) \endcode \return 3d
		   Vector of the pixel position of the center of this Sprite, in local space. */
		...
		/** Adds a sprite as a child of this Sprite.
			...
			New children are added at the bottom of the Sprite and will display on top of other children and this
		   Sprite. If you have a pointer to a Sprite, you can use addChildPtr(). \param newChild The Sprite to be added
		   as a child*/
		...
		/** Adds a sprite as a child of this Sprite.
			...
			Transform changes to this Sprite will also affect the new child.
			New children are added at the bottom of the Sprite and will display on top of other children and this
		   Sprite. If you have a reference to a Sprite, you can use addChild(). \param newChild The Sprite to be added
		   as a child*/
		template <class T>
		...
		/** The recommended "removal" API: removes this Sprite from parent; removes and deletes all it's children,
		   deletes this Sprite. In most cases, you will want to use this function to get rid of sprites. This removes
		   this sprite and it's children from the display list and clears all of that memory. Remember to clear any
		   pointers related to this Sprite or any references in any vectors you have saved yourself. */
		void release();
		...
		/** Whether or not to render this Sprite in the draw cycle; does not affect children.
			...
			\param transparent True means this sprite will not draw (but it's children could). False will render this
		   Sprite. */
		...
		/** Convert coordinate space from global (world) space to the local coordinate space of this Sprite. May not
		   ...
		   getParent()->globalToLocal(touchInfo.mCurrentGlobalPoint);
			}
			\endcode
		...
		/** Convert coordinate space from local coordinate space of this Sprite to global (world) coordinate space. May
		   ...
		   button on a panel like this: \code //In the button click handler ci::vec3 globalPosition =
		   localToGlobal(mTheLaunchButton.getPosition());
		...
		/** Recursively checks the Sprite hierarchy list for an enabled, visible sprite with a scale > 0.0 and any size
		   ...
		   global point to check. \return The Sprite that is the best candidate for touch picking. Can return nullptr if
		   there was no valid pick.*/
		virtual Sprite* getHit(const ci::vec3& point);

		/** Recursively checks the Sprite hierarchy list for an enabled, visible sprite with a scale > 0.0 and any size
		   for touch picking. This is for Perspective Sprites. Ortho Sprites use getHit() \param pick Some parameters
		   for perspective picking. \return The Sprite that is the best candidate for touch picking. Can return nullptr
		   if there was no valid pick.*/
		Sprite* getPerspectiveHit(CameraPick& pick);

		/** Touch handling enabling and disabling. Does not affect children. Sprite needs to be visible to handle
		   touches. \param flag True to enable touch handling, false disables this Sprite. */
		void enable(bool flag);

		/** Is this Sprite enabled to handle touch events? */
		bool isEnabled() const;

		/**	Defines the type of touch handling for this Sprite. Requires enable(true); to have been called at some
		   point. Constraints defined in multi_touch_constraints.h. For example: ds::ui::MULTITOUCH_INFO_ONLY. \param
		   constraints A bitmask of how touch is handled. For example: ds::ui::MULTITOUCH_CAN_POSITION |
		   ds::ui::MULTITOUCH_CAN_ROTATE */
		void enableMultiTouch(const BitMask& constraints);

		/** Disables specific MultiTouch handling, but leaves the enable() setting alone.
			After calling this, the Sprite will still grab touch events, but not do any specific multitouch handling
		   (like position, rotate, etc). Will send out tap and touchinfo callbacks. */
		...
		/** Set a lambda callback for each time this sprite gets interaction. Requires the sprite to be enabled. See
		 * TouchInfo for what data comes in. */
		...
		/** Get notified if a swipe occurred. This is called after the touch completes. Set the swipe threshold in
		 * engine.xml for how sensitive this is */
		void setSwipeCallback(const std::function<void(Sprite*, const ci::vec3&)>& func);

		/** Get notified when this sprite has been tapped. A tap is a single finger down and up within the tap
		   threshold. More than one finger at a time cancels taps. If doubleTapCallback has been set, waits for the
		   double tap time to expire before being notified. */
		...
		/** Used internally by touch process to set the destination sprite when this sprite is being checked for drag
		 * destinations */
		void setDragDestination(Sprite* dragDestination);
		/** Returns the sprite destination this sprite has been dragged over. Requires setDragDestinationCallback() to
		 * have been set. */
		...
		/** If the content of this sprite has been loaded. Base Sprite always returns true. Override for specific sprite
		 * types. */
		...
		virtual YGSize	  yogaMeasureFunc(YGNodeRef node, float width, YGMeasureMode widthMode, float height,
	...
	protected:
		...
		int		  mSpriteFlags;
		ci::vec3  mPosition, mCenter, mScale, mRotation;
	...
	/** Handle basic communication from the server. This creates sprites that don't exist, or updates ones that already
	   exist. Note: Cannot use Logs here, as including the logger doesn't compile. Also Note: Due to the way VS handles
	   templatization, this cannot be moved to the cpp file. Also also Note: It would be great if this weren't in the
	   Sprite header! */
	template <typename T>
	void Sprite::handleBlobFromServer(ds::BlobReader& r) {
		...
		if (attributey != SPRITE_ID_ATTRIBUTE) {
			std::cout
				<< "ERROR: Handle blob from server, attribute " << (int)attributey
				<< " is not the sprite attribute! This likely means you haven't installed your sprite type correctly."
				<< attributey << std::endl;
			return;
		}
		...
		if (s) {
			...
			if (!s) {
				std::cout << "ERROR: Failed to create sprite with id " << id
						  << " Ensure your sprite has a constructor that only takes a SpriteEngine as a parameter."
						  << std::endl;
				return;
			}
			s->setSpriteId(id);
			...
			/// clean up here.
			if (!s->mParent) {
				std::cout << "ERROR: No parent created for sprite id: " << id
						  << " Be sure you add your sprite to a parent directly after construction." << std::endl;
				// assert(false);
				delete s;
			}
		}
