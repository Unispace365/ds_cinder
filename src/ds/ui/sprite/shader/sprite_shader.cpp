...
const std::string DefaultVert = "#version 150\n"
...
const std::string NoImageVert = "#version 150\n"
								"uniform mat4       ciModelMatrix;\n"
								"uniform mat4       ciModelViewProjection;\n"
								"uniform vec4       uClipPlane0;\n"
								"uniform vec4       uClipPlane1;\n"
...
namespace ds { namespace ui {
	...
	SpriteShader::SpriteShader(const std::string& vert_memory, const std::string& frag_memory,
	...
	void SpriteShader::setShaders(const std::string& vert_memory, const std::string& frag_memory,
								  const std::string& shaderName) {
		if (mShader) {
			mShader.reset();
		}
		...
		try {
			...
			if (found == GlslProgs.end()) {
				...
				try {
					...
					if (geomFile.exists()) geomExists = true;
				} catch (std::exception&) {
					// swallow these, cause shaders could be loaded otherwise
				}
				if (exist) {
					if (geomExists) {
						mShader = ci::gl::GlslProg::create(ci::loadFile(vertLocation.c_str()),
														   ci::loadFile(fragLocation.c_str()),
														   ci::loadFile(geomLocation.c_str()));
					} else {
						mShader = ci::gl::GlslProg::create(ci::loadFile(vertLocation.c_str()),
														   ci::loadFile(fragLocation.c_str()));
					}
			...
			if (mName.empty()) {
				DS_LOG_WARNING_M("SpriteShader::loadShadersFromFile() on non empty name, did you intend that?",
								 SHADER_LOG);
			} else {
			...
			if (!ds::safeFileExistsCheck(vertLocation) || !ds::safeFileExistsCheck(fragLocation)) {
				...
				if (found == GlslProgs.end()) {
					mShader = ci::gl::GlslProg::create(
						ci::loadFile((mDefaultLocation + "/" + mDefaultName + ".vert").c_str()),
						ci::loadFile((mDefaultLocation + "/" + mDefaultName + ".frag").c_str()));
