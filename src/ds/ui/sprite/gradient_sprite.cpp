...
namespace ds { namespace ui {
	...
	Gradient& Gradient::makeH(SpriteEngine& e, const ci::ColorA& x1, const ci::ColorA& x2, Sprite* parent) {
		return makeAlloc<ds::ui::Gradient>(
			[&e, &x1, &x2]() -> ds::ui::Gradient* {
	...
	Gradient::Gradient(ds::ui::SpriteEngine& e, const ci::ColorA& tlColor, const ci::ColorA& trColor,
	...
	void Gradient::setColorsAll(const ci::ColorA& tlColor, const ci::ColorA& trColor, const ci::ColorA& brColor,
								const ci::ColorA& blColor) {
	...
	void Gradient::drawLocalClient() {
		...
		if (mRenderBatch) {
			...
			if (mCornerRadius > 0.0f) {
				...
				rect.colors(
					ci::ColorA(mTRColor, mTRColor.a * drawOpacity), ci::ColorA(mTLColor, mTLColor.a * drawOpacity),
					ci::ColorA(mBRColor, mBRColor.a * drawOpacity), ci::ColorA(mBLColor, mBLColor.a * drawOpacity));
				...
				rect.colors(
					ci::ColorA(mTLColor, mTLColor.a * drawOpacity), ci::ColorA(mTRColor, mTRColor.a * drawOpacity),
					ci::ColorA(mBRColor, mBRColor.a * drawOpacity), ci::ColorA(mBLColor, mBLColor.a * drawOpacity));
				rect.rect(ci::Rectf(0.0f, 0.0f, getWidth(), getHeight()));
				ci::gl::draw(rect);
			}
	...
	void Gradient::writeGradientColor(const DirtyState& dirty, const ci::ColorA& src, const char att,
									  ds::DataBuffer& buf) const {
		if (mDirty.has(dirty)) {
			buf.add(att);
			buf.add(src);
		}
	...
	void Gradient::tweenColorsAll(const ci::ColorA& tlColor, const ci::ColorA& trColor, const ci::ColorA& brColor,
								  const ci::ColorA& blColor, const float duration, const float delay,
