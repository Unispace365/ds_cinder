...
const std::string opacityPremultFrag = "uniform sampler2D	tex0;\n"
...
const std::string opacityFrag = "uniform sampler2D	tex0;\n"
								"in vec4			Color;\n"
								"in vec2			TexCoord0;\n"
								"out vec4	    	oColor;\n"
								"void main()\n"
...
const std::string vertShader = "uniform mat4		ciModelMatrix;\n"
...
namespace ds { namespace ui {
	...
	void Text::onBuildRenderBatch() {
		...
		if (getPerspective()) {
			drawRect = ci::Rectf(0.0f, static_cast<float>(mTexture->getHeight()),
	...
	ci::vec2 Text::getPositionForCharacterIndex(const int characterIndex) {
		...
		if (mPangoLayout) { // && !mText.empty()){
			...
			outputPos.y = (float)outputRectangle.y / (float)PANGO_SCALE +
						  (float)outputRectangle.height / (float)PANGO_SCALE / 4.0f;
		}
		return outputPos;
	}
	...
	bool Text::parseLists() {
		...
		std::string theList	   = mProcessedText.substr(firstInstance + 4, listEnd - firstInstance -
																			  4); // the minus four is to remove the tags
		...
		if (mNeedsFontUpdate || mNeedsMeasuring || mNeedsMarkupDetection) {
			...
			if (mNeedsFontUpdate || mNeedsFontSizeUpdate) {
				...
				PangoFontDescription* fontDescription = pango_font_description_from_string(
					mStyle.mFont.c_str()); // +" " + std::to_string(textSize)).c_str());
