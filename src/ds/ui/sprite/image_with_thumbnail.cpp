...
namespace ds { namespace ui {
	...
	ImageWithThumbnail::ImageWithThumbnail(SpriteEngine& engine, const std::string& filename, const int flags,
										   float fadeDuration)
	...
	ImageWithThumbnail::ImageWithThumbnail(SpriteEngine& engine, const ds::Resource::Id& resourceId, const int flags,
										   float fadeDuration)
	...
	ImageWithThumbnail::ImageWithThumbnail(SpriteEngine& engine, const ds::Resource& resource, const int flags,
										   float fadeDuration)
	  : inherited(engine, flags)
	  , mThumbnail(nullptr)
	  , mFadeDuration(fadeDuration) {
	...
	void ImageWithThumbnail::setCircleCropRect(const ci::Rectf& rect) {
		...
		if (mThumbnail) {
			...
			ci::Rectf scaleRect(rect.x1 * inverseThumbScale, rect.y1 * inverseThumbScale, rect.x2 * inverseThumbScale,
