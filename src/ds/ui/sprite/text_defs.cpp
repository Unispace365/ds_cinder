...
namespace ds { namespace ui {
	...
	TextStyle::TextStyle(const std::string& font, const std::string& configName, const double size,
						 const double leading, const double letterSpacing, const ci::ColorA& c,
						 const ds::ui::Alignment::Enum& alignment)
	  : mFont(font)
	  , mName(configName)
	  , mSize(size)
	  , mLeading(leading)
	...
	ds::ui::TextStyle TextStyle::textStyleFromSetting(ds::ui::SpriteEngine& engine, const std::string& settingString) {
		...
		for (auto it : settings) {
			...
			/// note that we don't just split on ":" for the key/values because a lot of text style names have colons in
			/// them
	...
	std::string TextStyle::settingFromTextStyle(ds::ui::SpriteEngine& engine, TextStyle& theStyle,
		...
		if (theStyle.mFitMinTextSize != aDefaultStyle.mFitMinTextSize ||
			...
			items.emplace_back("fit_size_range:" + std::to_string(theStyle.mFitMinTextSize) + ", " +
							   std::to_string(theStyle.mFitMaxTextSize));
		}
