...
namespace ds { namespace ui {
	...
	YGSize _yogaMeasureFunc(YGNodeRef node, float width, YGMeasureMode widthMode, float height,
	...
	void Sprite::init(const ds::sprite_id_t id) {
		...
		mServerColor =
			ci::ColorA(static_cast<float>(ci::randFloat(0.5, 1.0f)), static_cast<float>(ci::randFloat(0.5, 1.0f)),
					   static_cast<float>(ci::randFloat(0.5, 1.0f)), 0.4f);
	...
	void Sprite::drawClient(const ci::mat4& trans, const DrawParams& drawParams) {
		...
		auto renderInternal = [&]() {
			...
			if (!mIsRenderFinalToTexture &&
				(mSpriteFlags & CLIP_F) !=
					0) { // Clipping is implicit when rendering to an FBO, only set clipping if we aren't
				const ci::Rectf& clippingBounds = getClippingBounds(drawParams.mClippingParent);
				clip_plane::enableClipping(clippingBounds.getX1(), clippingBounds.getY1(), clippingBounds.getX2(),
										   clippingBounds.getY2());
			}
	...
	void Sprite::doSetRotation(const ci::vec3& rot) {
		if (math::isEqual(mRotation.x, rot.x) && math::isEqual(mRotation.y, rot.y) &&
			math::isEqual(mRotation.z, rot.z) && mDegree == 0.0f)
			return;
	...
	void Sprite::buildTransform() const {
		...
		if (!mDoSpecialRotation) {
			mTransformation =
				glm::rotate(mTransformation, mRotation.x * math::DEGREE2RADIAN, glm::vec3(1.0f, 0.0f, 0.0f));
			mTransformation =
				glm::rotate(mTransformation, mRotation.y * math::DEGREE2RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
			mTransformation =
				glm::rotate(mTransformation, mRotation.z * math::DEGREE2RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
		} else {
	...
	bool Sprite::contains(const ci::vec3& point, const float pad) const {
		...
		return (dot1 >= 0 && dot2 >= 0 && dot1 <= dot3 && dot2 <= dot4);
	}
	...
	Sprite* Sprite::getHit(const ci::vec3& point) {
		...
		if (isEnabled() && contains(point) && getInnerHit(point)) return this;
	...
	Sprite* Sprite::getPerspectiveHit(CameraPick& pick) {
		...
		for (auto it = mSortedTmp.rbegin(), it2 = mSortedTmp.rend(); it != it2; ++it) {
			...
			if (hit) {
				...
				// std::cout << "Found pick candidate: " << hit->getParent()->localToGlobal(hit->getPosition()).z << " "
				// << hit->getId() << std::endl; return hit;
			}
		}
	...
	void Sprite::doubleTap(const ci::vec3& tapPos) {
		if (mDoubleTapCallback) mDoubleTapCallback(this, tapPos);
	}
	...
	void Sprite::processTouchInfoCallback(const TouchInfo& touchInfo) {
		if (mProcessTouchInfoCallback) mProcessTouchInfoCallback(this, touchInfo);
	}
	...
	bool Sprite::checkBounds() const {
		...
		for (int i = 1; i < 4; ++i) {
			if (positions[i].x < spriteMinX) spriteMinX = positions[i].x;
			if (positions[i].y < spriteMinY) spriteMinY = positions[i].y;
			if (positions[i].x > spriteMaxX) spriteMaxX = positions[i].x;
			if (positions[i].y > spriteMaxY) spriteMaxY = positions[i].y;
		}
		...
		if (spriteMinX > screenMaxX) return false;
		if (spriteMaxX < screenMinX) return false;
		if (spriteMinY > screenMaxY) return false;
		if (spriteMaxY < screenMinY) return false;

		for (int i = 0; i < 4; ++i) {
			if (positions[i].x >= screenMinX && positions[i].x <= screenMaxX && positions[i].y >= screenMinY &&
				positions[i].y <= screenMaxY) {
				mInBounds = true;
				return true;
			}
		}
		...
		for (int i = 0; i < 4; ++i) {
			if (screenpos[i].x >= spriteMinX && screenpos[i].x <= spriteMaxX && screenpos[i].y >= spriteMinY &&
				screenpos[i].y <= spriteMaxY) {
				mInBounds = true;
				return true;
			}
		}
		...
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
				if (math::intersect2D(screenpos[i % 4], screenpos[(i + 1) % 4], positions[i % 4],
									  positions[(i + 1) % 4])) {
					mInBounds = true;
					return true;
				}
	...
	void Sprite::updateCheckBounds() const {
		if (mBoundsNeedChecking) checkBounds();
	}
	...
	void Sprite::setBaseShader(const std::string& vertShader, const std::string& fragShader,
							   const std::string& shadername, bool applyToChildren) {
		mNeedsBatchUpdate = true;
	...
	void Sprite::setupFinalRenderBuffer() {
		if (mIsRenderFinalToTexture) {
			...
			if (mOutputFbo && mOutputFbo->getWidth() == (int)getWidth() &&
				mOutputFbo->getHeight() == (int)getHeight()) {
				// Reuse
				// DS_LOG_INFO("Reusing my FBO :)");
			} else if (getWidth() > 1.0f && getHeight() > 1.0f) {
				mOutputFbo =
					ci::gl::Fbo::create(static_cast<int>(getWidth()), static_cast<int>(getHeight()), mFboFormat);
	...
	void Sprite::computeClippingBounds(ds::ui::Sprite* clippingParent) {
		if (getClipping()) {
			...
			if (clippingParent) {
				...
				while (curSprite) {
					if (curSprite->getClipping()) outerClippedSprite = curSprite;
	...
	void Sprite::makeSortedChildren() {
		...
		std::sort(mSortedTmp.begin(), mSortedTmp.end(),
	...
	void Sprite::setSpriteOrder(const std::vector<sprite_id_t>& order) {
		for (auto it = order.begin(), end = order.end(); it != end; ++it) {
			...
			auto			  found = std::find_if(mChildren.begin(), mChildren.end(),
												   [id](Sprite* s) -> bool { return s && s->getId() == id; });
			if (found != mChildren.end()) {
				Sprite* s(*found);
				mChildren.erase(found);
	...
	void Sprite::passTouchToSprite(Sprite* destinationSprite, const TouchInfo& touchInfo) {
		...
		newTouchInfo.mCurrentGlobalPoint = localToGlobal(
			ci::vec3(-10000.0f, -10000.0f,
					 0.0f)); // make sure we touch up outside the sprite area, so buttons don't think they're hit
	...
	void Sprite::setFlexboxAutoSizes() {
		...
		if (mYogaNode->getChildren().size() == 0) {
			if (YGNodeStyleGetWidth(mYogaNode).unit == YGUnitAuto &&
				(YGNodeStyleGetMinWidth(mYogaNode).unit == YGUnitAuto ||
				 YGNodeStyleGetMinWidth(mYogaNode).unit == YGUnitUndefined) &&
				(YGNodeStyleGetMaxWidth(mYogaNode).unit == YGUnitAuto ||
				 YGNodeStyleGetMaxWidth(mYogaNode).unit == YGUnitUndefined)) {
				YGNodeStyleSetWidth(mYogaNode, getScaleWidth());
				YGNodeMarkDirty(mYogaNode);
			}

			if (YGNodeStyleGetHeight(mYogaNode).unit == YGUnitAuto &&
				(YGNodeStyleGetMinHeight(mYogaNode).unit == YGUnitAuto ||
				 YGNodeStyleGetMinHeight(mYogaNode).unit == YGUnitUndefined) &&
				(YGNodeStyleGetMaxHeight(mYogaNode).unit == YGUnitAuto ||
				 YGNodeStyleGetMaxHeight(mYogaNode).unit == YGUnitUndefined)) {
	...
	void Sprite::writeState(std::ostream& s, const size_t tab) const {
		...
		const auto getNameStr = [this, eraseAll] {
			...
			return boost::format("%s%s%s") % typeName % (spriteName.empty() ? "" : std::string("#") + spriteName) %
				   xmlStr;
		};
		...
		s << nameStr
		  << boost::format("  [%5.0f, %5.0f] @ (%6.1f, %6.1f)   opacity=%4.2f %s %s %s\n") % getWidth() % getHeight() %
				 pos.s % pos.y % getOpacity() % (this->visible() ? "        " : "<HIDDEN>") %
				 (this->getClipping() ? "<CLIP>" : "      ") % (this->isEnabled() ? "<ENABLED>" : "         ");
		/*
		s << "CLASS NAME= " << typeid(this).name() << "ID=" << mId << " flags=" << mSpriteFlags << " pos=" << mPosition
		<< " size=[" << mWidth << "x" << mHeight << "x" << mDepth << "] scale=" << mScale << " cen=" << mCenter << "
		rot=" << mRotation << " clip=" << mClippingBounds << std::endl; for (size_t k=0; k<tab+2; ++k) s << "\t"; s <<
		"STATE opacity=" << mOpacity << " use_shader=" << mUseShaderTexture << " use_depthbuffer=" << mUseDepthBuffer <<
		" last_w=" << mLastWidth << " last_h=" << mLastHeight << std::endl; for (size_t k=0; k<tab+2; ++k) s << "\t"; s
		<< "STATE need_bounds_check=" << mBoundsNeedChecking << " in_bounds=" << mInBounds << " check_bounds=" <<
		mCheckBounds << " clip_dirty=" << mClippingBoundsDirty << " update_transform=" << mUpdateTransform << std::endl;
		// Transform
	...
	YGSize Sprite::yogaMeasureFunc(YGNodeRef node, float width, YGMeasureMode widthMode, float height,
								   YGMeasureMode heightMode) {
		YGSize retVal;
