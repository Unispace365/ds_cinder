...
namespace ds { namespace ui {
	...
	ControlSlider::ControlSlider(ds::ui::SpriteEngine& engine, const bool vertical, const float uiSize,
	...
	void ControlSlider::setSliderUpdatedCallback(
		std::function<void(const double scrollPercent, const double sliderValue, const bool finishedAdjusting)> func) {
	...
	void ControlSlider::updateNubPosition() {
		if (mNub) {
			if (mVertical) {
				...
				if (getPerspective()) {
					mNub->setPosition(mTouchPadding,
									  (1.0f - (float)mSliderPercent) * (getHeight() - mNub->getHeight()));
				} else {
					mNub->setPosition(mTouchPadding,
									  (float)mSliderPercent * getHeight() - (float)mSliderPercent * mNub->getHeight());
				}
			} else {

				//	mNub->setSize(mNub->getWidth(), getHeight());

				mNub->setPosition(getWidth() * (float)mSliderPercent - (float)mSliderPercent * mNub->getWidth(),
								  getHeight() / 2.0f - mNub->getHeight() / 2.0f);
			}
		}
