...
namespace ds { namespace ui {
	...
	class ControlSlider : public ds::ui::Sprite {
	  public:
		...
		ControlSlider(ds::ui::SpriteEngine& engine, const bool vertical = true, const float uiSize = 7.5f,
					  const float slideLength = 100.0f);
		...
		void setSliderUpdatedCallback(
			std::function<void(const double sliderPercent, const double sliderValue, const bool finishedAdjusting)>
				func);
