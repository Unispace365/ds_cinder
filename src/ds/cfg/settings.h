...
class Settings {
  public:
	...
	struct Setting {
		...
		/// Optional type hinting for GUI editors or human readers. All getters are still available regardless of
		/// this value
		std::string mType;
	...
	/// If the setting doesn't exist when you get it, it will be created. Supplying the default value will apply
	/// that value to the new setting
	...
	/// Returns the index of the name of the setting in the vector of all settings (not the index of the same
	/// setting) Return -1 if the setting isn't there or there are no settings NOTE: This is NOT NOT NOT the same
	/// index as above. This is the index of ALL setting names. The indices above are for settings with the same
	/// name
	int getSettingIndex(const std::string& name) const;

	/// Iterate over all the settings, optionally filtering by a specific type
	/// The order they are returned is the overall order they were originally read in (the same order returned by
	/// getReadSortedSettings())
	...
	/// Returns a reference to a vector of all Settings sorted by the order they were added (or if their mReadIndex
	/// value was changed later) This vector may change at any time, so careful here
  ...
  protected:
	...
	/// The inner vector is for a series of settings with the same name (to support the index calls in the
	/// getSetting() calls)
