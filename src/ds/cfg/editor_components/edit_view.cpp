...
namespace ds { namespace cfg {
	...
	EditView::EditView(ds::ui::SpriteEngine& e)
	  ...
	  , mPossibleIndex(0) {
		...
		mCancelButton->setTapCallback([this](ds::ui::Sprite* bs, const ci::vec3& pos) { stopEditing(); });
		...
		mApplyButton->setTapCallback([this](ds::ui::Sprite* bs, const ci::vec3& pos) { applySetting(true); });
		...
		mSaveButton->setTapCallback([this](ds::ui::Sprite* bs, const ci::vec3& pos) { applySetting(false); });
	...
	void EditView::setSetting(Settings::Setting* theSetting, const std::string& parentSetingsName) {
		...
		if (!mSettingName || !mSettingValue || !mTheSetting || !mSettingComment || !mSettingDefault || !mSettingMin ||
			!mSettingMax || !mSettingSource || !mSettingPossibles)
			return;
		...
		if (mEntryEditor && mKeyboard && mSlider && mTheSetting) {
			...
			mEntryEditor->setNativeKeyboardCallback([this](ci::app::KeyEvent& event) -> bool {
				...
				if (mNextSettingCallback &&
					(event.getCode() == ci::app::KeyEvent::KEY_DOWN || event.getCode() == ci::app::KeyEvent::KEY_TAB)) {
					applySetting(false);
					mNextSettingCallback(true);
					return true;
				}

				if (mNextSettingCallback && (event.getCode() == ci::app::KeyEvent::KEY_RETURN ||
											 event.getCode() == ci::app::KeyEvent::KEY_KP_ENTER)) {
					applySetting(true);
					return true;
				}

				if (mNextSettingCallback && event.getCode() == ci::app::KeyEvent::KEY_UP) {
			...
			if (theSetting->mType == ds::cfg::SETTING_TYPE_BOOL) {
				...
				mSlider->setSliderLimits(ds::string_to_double(theSetting->mMinValue),
										 ds::string_to_double(theSetting->mMaxValue));
