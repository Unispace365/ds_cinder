	...
	void SettingsEditor::showSettings(const std::string theSettingsName) {
		...
		mCurrentSettings->forEachSetting([this](ds::cfg::Settings::Setting& setting) {
			...
			ei->setProcessTouchCallback([this](ds::ui::Sprite* bs, const ds::ui::TouchInfo& ti) {
				if (bs && mSettingsLayout && ti.mPhase == ds::ui::TouchInfo::Moved &&
					glm::distance(ti.mCurrentGlobalPoint, ti.mStartPoint) > mEngine.getMinTapDistance()) {
					bs->passTouchToSprite(mSettingsLayout, ti);
				}
			});
			ei->setTapCallback([this, ei](ds::ui::Sprite* bs, const ci::vec3& pos) { editProperty(ei); });
			if (mSettingsLayout) {
		...
		// setPosition(ci::app::getWindowBounds().getWidth() - mPrimaryLayout->getWidth(), 0.0f);//
		// mEngine.getSrcRect().getY1());
		...
		theButton->setTapCallback([this, savePath, saveName, theButton](ds::ui::Sprite* bs, const ci::vec3& pos) {
			if (mCurrentSettings) {
				...
				try {
					...
					thePath.makeParent();
					ShellExecute(NULL, L"open", ds::wstr_from_utf8(thePath.toString()).c_str(), NULL, NULL,
								 SW_SHOWDEFAULT);
				} catch (std::exception) {
				...
				theButton->callAfterDelay([this, saveName, theButton] { theButton->setText("Save in " + saveName); },
										  3.0f);
			}
		});
				...
				if (mSettingsLayout && mPrimaryLayout) {
					for (auto it : mSettingItems) {
						...
						if (it->getSettingName() == theSetting->mName || it->isDerived()) {
							auto& update_setting = mCurrentSettings->getSetting(
								it->getSettingName(), 0); // We'll need to handle indexes at some point.
