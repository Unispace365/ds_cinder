...
namespace ds { namespace gl {
	...
	void Uniform::setInternal(const std::string& key, const UniformVisitor::SupportedVariants& val,
...
#ifdef _DEBUG
		std::cout << "Uniform::set(const std::string&, const float *, int) -- this variant should be deprecated. Use "
					 "set(const std::string&, const std::vector<float>&) instead. This version is slow and copies the "
					 "data in (which it should, but would be confusing if you were thinking you could free the array "
					 "after the call)"
				  << std::endl;
#endif
		if (data && count > 0) {
