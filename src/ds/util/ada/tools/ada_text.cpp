...
std::string purifyString(const std::string& _string) {
	...
	for (std::string::iterator it = std.begin(), itEnd = std.end(); it != itEnd; ++it) {
		if (static_cast<uint32_t>(*it) < 32 || static_cast<uint32_t>(*it) > 127 || *it == '.' || *it == '-' ||
			*it == '\\' || *it == '/') {
			(*it) = '_';
		}
	}
...
int count_buffers(const std::string& _source) {
	...
	// Regext to search for #ifdef BUFFER_[NUMBER], #if defined( BUFFER_[NUMBER] ) and #elif defined( BUFFER_[NUMBER] )
	// occurences
	std::regex re(
		R"((?:^\s*#if|^\s*#elif)(?:\s+)(defined\s*\(\s*BUFFER_)(\d+)(?:\s*\))|(?:^\s*#ifdef\s+BUFFER_)(\d+))");
	std::smatch match;
...
bool check_for_background(const std::string& _source) {
	...
	std::regex re(
		R"((?:^\s*#if|^\s*#elif)(?:\s+)(defined\s*\(\s*BACKGROUND)(?:\s*\))|(?:^\s*#ifdef\s+BACKGROUND)|(?:^\s*#ifndef\s+BACKGROUND))");
	std::smatch match;
...
bool check_for_floor(const std::string& _source) {
	...
	std::regex re(
		R"((?:^\s*#if|^\s*#elif)(?:\s+)(defined\s*\(\s*FLOOR)(?:\s*\))|(?:^\s*#ifdef\s+FLOOR)|(?:^\s*#ifndef\s+FLOOR))");
	std::smatch match;
...
bool check_for_postprocessing(const std::string& _source) {
	...
	std::regex re(
		R"((?:^\s*#if|^\s*#elif)(?:\s+)(defined\s*\(\s*POSTPROCESSING)(?:\s*\))|(?:^\s*#ifdef\s+POSTPROCESSING)|(?:^\s*#ifndef\s+POSTPROCESSING))");
	std::smatch match;
...
int count_convolution_pyramid(const std::string& _source) {
	...
	// occurences
	std::regex re(
		R"((?:^\s*#if|^\s*#elif)(?:\s+)(defined\s*\(\s*CONVOLUTION_PYRAMID_)(\d+)(?:\s*\))|(?:^\s*#ifdef\s+CONVOLUTION_PYRAMID_)(\d+))");
	std::smatch match;
...
bool check_for_convolution_pyramid_algorithm(const std::string& _source) {
	...
	std::regex re(
		R"((?:^\s*#if|^\s*#elif)(?:\s+)(defined\s*\(\s*CONVOLUTION_PYRAMID_ALGORITHM)(?:\s*\))|(?:^\s*#ifdef\s+CONVOLUTION_PYRAMID_ALGORITHM)|(?:^\s*#ifndef\s+CONVOLUTION_PYRAMID_ALGORITHM))");
	std::smatch match;
...
bool check_for_pattern(const std::string& _str) {
	return (_str.find('*') != std::string::npos) || (_str.find('?') != std::string::npos);
}
