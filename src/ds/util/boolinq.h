...
class Linq {
  ...
  public:
	...
	T next() { return nextFunc(storage); }
	...
	Linq<std::tuple<Linq<S, T>, int>, _TRet> select_i(F apply) const {
		return Linq<std::tuple<Linq<S, T>, int>, _TRet>(std::make_tuple(*this, 0),
	...
	Linq<std::tuple<Linq<S, T>, Linq<S2, T2>, bool>, T> concat(const Linq<S2, T2>& rhs) const {
		return Linq<std::tuple<Linq<S, T>, Linq<S2, T2>, bool>, T>(
			std::make_tuple(*this, rhs, false), [](std::tuple<Linq<S, T>, Linq<S2, T2>, bool>& tuple) {
	...
	Linq<std::tuple<Linq<S, T>, _TRet, int, bool>, _TRetVal> selectMany_i(F apply) const {
		return Linq<std::tuple<Linq<S, T>, _TRet, int, bool>, _TRetVal>(
			std::make_tuple(*this, _TRet(), 0, true), [apply](std::tuple<Linq<S, T>, _TRet, int, bool>& tuple) {
				Linq<S, T>& linq	 = std::get<0>(tuple);
	...
	template <typename F, typename _TKey = typename std::result_of<F(T)>::type,
			  typename _TValue = Linq<std::tuple<Linq<S, T>, int>, T> // where(predicate)
			  >
	Linq<std::tuple<Linq<S, T>, Linq<S, T>, std::unordered_set<_TKey>>, std::pair<_TKey, _TValue>>
	...
	Linq<std::tuple<std::vector<T>, _TIter, bool>, T> orderBy(F transform) const {
		...
		std::sort(items.begin(), items.end(),
				  [transform](const T& a, const T& b) { return transform(a) < transform(b); });

		return Linq<std::tuple<std::vector<T>, _TIter, bool>, T>(std::make_tuple(items, _TIter(), false),
	...
	Linq<std::tuple<std::list<T>, _TIter, bool>, T> reverse() const {
		return Linq<std::tuple<std::list<T>, _TIter, bool>, T>(std::make_tuple(toStdList(), _TIter(), false),
	...
	_TRet sum(F transform) const {
		return aggregate<_TRet>(_TRet(),
								[transform](_TRet accumulator, T value) { return accumulator + transform(value); });
	}
	...
	int count(std::function<bool(T)> predicate) const { return where(predicate).count(); }
	...
	T elementAt(int index) const { return skip(index).next(); }

	T first(std::function<bool(T)> predicate) const { return where(predicate).next(); }

	T first() const { return Linq<S, T>(*this).next(); }
	...
	std::set<T> toStdSet() const {
		std::set<T> items;
		for_each([&items](T value) { items.insert(value); });
		return items;
	}
	...
	Linq<std::tuple<Linq<S, T>, BytesDirection, int>, TRet> unbytes(BytesDirection direction = BytesFirstToLast) const {
		return Linq<std::tuple<Linq<S, T>, BytesDirection, int>, TRet>(
			std::make_tuple(*this, direction, 0), [](std::tuple<Linq<S, T>, BytesDirection, int>& tuple) {
				Linq<S, T>&		linq		   = std::get<0>(tuple);
				BytesDirection& bytesDirection = std::get<1>(tuple);
				// int &index = std::get<2>(tuple);
	...
	Linq<std::tuple<Linq<S, T>, BytesDirection, BitsDirection, T, int>, int>
	bits(BitsDirection bitsDir = BitsHighToLow, BytesDirection bytesDir = BytesFirstToLast) const {
	...
	Linq<std::tuple<Linq<S, T>, BytesDirection, BitsDirection, int>, TRet>
	unbits(BitsDirection bitsDir = BitsHighToLow, BytesDirection bytesDir = BytesFirstToLast) const {
...
Linq<std::pair<T, int>, T> repeat(const T& value, int count) {
	return Linq<std::pair<T, int>, T>(std::make_pair(value, count), [](std::pair<T, int>& pair) {
		if (pair.second > 0) {
			pair.second--;
			return pair.first;
		}
...
Linq<std::tuple<T, T, T>, T> range(const T& start, const T& end, const T& step) {
	return Linq<std::tuple<T, T, T>, T>(std::make_tuple(start, end, step), [](std::tuple<T, T, T>& tuple) {
