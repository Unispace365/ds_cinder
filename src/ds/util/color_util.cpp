...
std::string ARGBToHex(ci::ColorA theColor) {
	return ARGBToHex((int)(theColor.a * 255.0f), (int)(theColor.r * 255.0f), (int)(theColor.g * 255.0f),
					 (int)(theColor.b * 255.0f));
}

std::string ARGBToHex(int aNum, int rNum, int gNum, int bNum) {
	std::stringstream ss;
	ss << '#' << std::hex << std::setfill('0') << std::setw(2) << (aNum & 0xff) << std::setw(2) << (rNum & 0xff)
	   << std::setw(2) << (gNum & 0xff) << std::setw(2) << (bNum & 0xff);
...
std::string RGBToHex(int rNum, int gNum, int bNum) {
	std::stringstream ss;
	ss << '#' << std::hex << std::setfill('0') << std::setw(2) << (rNum & 0xff) << std::setw(2) << (gNum & 0xff)
...
ci::ColorA parseHexColor(const std::string& color) {
	...
	if (boost::starts_with(s, "#")) boost::erase_head(s, 1);

	std::stringstream converter(s);
	unsigned int	  value;
	converter >> std::hex >> value;

	float a = (s.length() > 6) ? ((value >> 24) & 0xFF) / 255.0f : 1.0f;
