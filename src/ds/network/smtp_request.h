...
namespace ds { namespace net {
	...
	class SMTPRequest {
	  public:
		...
		/// userInfo, first is email address for sender, second is email address for receiver, and following is email
		/// address for cc. format is "<example@email.com>" sInput is the content for the email, include
		/// from,to,cc,subject,content body. details see SMTP sample userpwd is the username and password for SMTP
		/// account. format is "user:pwd"


		void makePostRequest(const std::string serverUrl, const std::vector<std::string> userInfo,
		...
		void setReplyFunction(std::function<void(const bool errored, const std::string& reply)> func) {
			mReplyFunction = func;
		}
