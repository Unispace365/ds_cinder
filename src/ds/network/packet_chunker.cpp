...
namespace ds { namespace net {
	...
	void Chunker::Chunkify(const char* src, unsigned size, unsigned groupId, std::vector<std::string>& dst) {
		...
		for (i = 0; i < numIterations; ++i) {
			...
			std::copy(reinterpret_cast<char*>(&header), reinterpret_cast<char*>(&header) + sizeof(ChunkHeader),
					  dst[i].begin());
			std::copy(mCompressedBuffer.begin() + pos, mCompressedBuffer.begin() + pos + chunkSize,
					  dst[i].begin() + sizeof(ChunkHeader));
			pos += chunkSize;
		}
		...
		if (excess) {
			...
			std::copy(reinterpret_cast<char*>(&header), reinterpret_cast<char*>(&header) + sizeof(ChunkHeader),
					  dst[i].begin());
			std::copy(mCompressedBuffer.begin() + pos, mCompressedBuffer.begin() + pos + excess,
					  dst[i].begin() + sizeof(ChunkHeader));
			pos += chunkSize;
		}
	}
	...
	void DeChunker::addChunkToGroup(DeChunkStats& stats, const char* chunk, unsigned size) {
		...
		if (found == stats.mIdsMissing.end()) return;
	...
	bool DeChunker::getNextGroup(std::string& dst) {
		if (!mGroupsAvailable.empty() && !mGroupsReceived.empty()) {
			...
			if (gr != mGroupsReceived.end()) {
				...
				if (mDataChunks[groupId].mData.get()) {
					snappy::Uncompress(mDataChunks[groupId].mData.get()->c_str(),
									   mDataChunks[groupId].mData.get()->size(), &dst);
				}
		...
		std::cout << "get next group is borking: " << mGroupsAvailable.empty() << " " << mGroupsReceived.empty()
				  << std::endl;
