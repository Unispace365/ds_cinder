 ...
 * \brief Wraps NodeWatcher to implement a delay on messages, so if many messages come down the pipe in quick
 *succession, the query only happens after the end of the burst This prevents querying a lot of times unnecessarily.
 */
class DelayedNodeWatcher : public ds::AutoUpdate {
  public:
	...
	/** Waits until there is a gap in messages to callback. No message-specific info here, generally just query
	 * everything from this message */
	void setDelayedNodeCallback(const std::function<void()>&);

	/** Waits until there is a gap in messages to callback. This one has message info in case you want it to query
	 * specific tables */
	void setDelayedMessageNodeCallback(const std::function<void(const NodeWatcher::Message&)>&);

	/** How long to wait since the last message to send out a callback. Delay is extended if a node message comes in
	 * before the delay is up. */
