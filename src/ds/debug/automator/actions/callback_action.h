...
public:
	CallbackActionFactory(std::function<void(void)> callback, const float minFrequency = 0.0f, const float maxFrequency = 1.0f)
		:mCallback(callback)		
...
{
public:
	CallbackAction(std::vector<int> &freeList, ds::ui::SpriteEngine& engine, const ci::Rectf& frame, const std::function<void(void)>& callback, const float minTime, const float floatMaxTime);
