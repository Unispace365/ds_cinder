...
AutomatorApp::AutomatorApp()
  ...
  , mAutomator(mEngine) {
	registerKeyPress(
		"Toggle automator", [this] { mAutomator.toggleActive(); }, ci::app::KeyEvent::KEY_p);
}
...
void AutomatorApp::setupServer() {
	...
	mSprite1.setProcessTouchCallback(
		[this](ds::ui::Sprite* sp, const ds::ui::TouchInfo& ti) { handleTouchUno(ti.mCurrentGlobalPoint); });
	...
	mSprite2.setProcessTouchCallback(
		[this](ds::ui::Sprite* sp, const ds::ui::TouchInfo& ti) { handleTouchDuo(ti.mCurrentGlobalPoint); });
	rootSprite.addChild(mSprite2);

	auto callback = [this]() {
		recenterSprites();
	};
	mAutomator.addFactory(
		std::shared_ptr<ds::debug::BaseActionFactory>(new ds::debug::CallbackActionFactory(callback, 1.0f, 10.0f)));
}
