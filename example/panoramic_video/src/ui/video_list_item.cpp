...
void VideoListItem::setState(const int buttonState) {
	if (mBackground) {
		ci::Color normalColor =
			mEngine.getAppSettings().getColor(mEngine, "info_list:item:normal_color", 0, ci::Color::white());
		ci::Color highliColor =
			mEngine.getAppSettings().getColor(mEngine, "info_list:item:highli_color", 0, ci::Color::white());
		if (buttonState == 1) {
