	...
	, mEventClient(g.mEngine.getNotifier(), [this](const ds::Event *m){ if(m) this->onAppEvent(*m); })
{
	...
	mFileList->setCreateItemCallback([this, itemSize]()->ds::ui::Sprite* {
		return new VideoListItem(mGlobals, mFileList->getWidth(), itemSize);
	});
	...
	mCloseButton->setClickFn([this]{
		animateOff();
	});

	ds::ui::Sprite* backy = new ds::ui::Sprite(mEngine, mCloseButton->getScaleWidth() * 2.0f, mCloseButton->getScaleHeight() * 2.0f);
	...
	setData();
	setPosition(mEngine.getWorldWidth() / 2.0f - getWidth()/2.0f, mEngine.getWorldHeight() / 2.0f - getHeight() / 2.0f);
}
