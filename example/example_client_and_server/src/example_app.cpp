...
void CsApp::setupServer() {
	...
	imgSprite->setProcessTouchCallback(
		[this](ds::ui::Sprite* bs, const ds::ui::TouchInfo& ti) { imgSprite->sendToFront(); });
	...
	kittySprite->setProcessTouchCallback(
		[this, kittySprite](ds::ui::Sprite* bs, const ds::ui::TouchInfo& ti) { kittySprite->sendToFront(); });
	...
	child->setProcessTouchCallback(
		[this, child](ds::ui::Sprite* bs, const ds::ui::TouchInfo& ti) { child->sendToFront(); });
	rootSprite.addChild(*child);

	// Example text sprite
	mTexty = new ds::ui::Text(mEngine);
	mTexty->setFont("arial", 16.0f);
	mTexty->setText("<span weight='bold'>Welcome to the server client setup in ds_cinder!</span> \t\t This text should "
					"be displaying across both the <span style='italic'>client</span> and the server!");
...
void CsApp::recreateText() {
	...
	if (mTexty) {
		...
		mTexty->setFont("arial", 16.0f);
		std::stringstream ss;
		ss << "<span weight='bold'>Hey</span> there" << ci::randFloat() << " " << std::endl
		   << ci::randFloat() << " " << ci::randFloat();
		mTexty->setText(ss.str());
		mTexty->setText("Well hello there");
		mTexty->setText("<span weight='bold'>Welcome to the server client setup in ds_cinder!</span> \t\t This text "
						"should be displaying across both the <span style='italic'>client</span> and the server!");
		mTexty->setPosition(getWindowWidth() * 0.25f, getWindowHeight() * 0.75f);
		mTexty->getWidth();
		mTexty->enable(true);
