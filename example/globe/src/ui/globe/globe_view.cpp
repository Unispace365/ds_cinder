...
namespace ds {
	...
	namespace ui {
		...
		GlobeView::GlobeView(SpriteEngine& e, float minTilt, float maxTilt, float radius, int resolution)
			...
			, mMinTilt(minTilt)
			, mMaxTilt(maxTilt)
			, mPinManager(ds::ui::Sprite::makeAlloc<ds::ui::GlobePinManager>([this]()->ds::ui::GlobePinManager* {return new ds::ui::GlobePinManager(mEngine); }, this))
			, mTouchGrabber(ds::ui::Sprite::makeAlloc<ds::ui::Sprite>([this]()->ds::ui::Sprite* {return new ds::ui::Sprite(mEngine); }, this))
			, mXMomentum(e)
			, mYMomentum(e)
			, mGlobeRadius(radius)
			...
			, mFocusOffset(ci::vec2(0.0f,0.0f))
		{
			...
			mEarth = ci::gl::Batch::create(ci::geom::Sphere().radius(mGlobeRadius).subdivisions(mMeshResolution), mEarthShader);

			mOccluder = ci::gl::Batch::create(ci::geom::Circle().radius(mGlobeRadius + 40), ci::gl::getStockShader(ci::gl::ShaderDef().color()));
			...
			
			{
				...
				mGlow = ci::gl::Batch::create(ci::geom::Plane().size(ci::vec2(mGlobeRadius*3.4, mGlobeRadius*3.4)), mGlowShader);
			}
		...
		void GlobeView::onUpdateServer(const ds::UpdateParams& updateParams) {
			...
			//Limit xRotation;
			auto rot = ci::vec3(mGlobeRotation.x + mXMomentum.getDelta(), mGlobeRotation.y + orbit + mYMomentum.getDelta(), 0.0f);
