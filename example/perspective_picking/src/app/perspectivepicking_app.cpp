...
void PerspectivePicking::update() {
	...
	if (mTopLeftMarker && mTouchTestDuo) {
		...
		const float w = mTouchTestDuo->getScaleWidth(), h = mTouchTestDuo->getScaleHeight();
		...
		ptA_s =
			perspCamera.worldToScreen(ci::vec3(50.0f, 50.0f, 50.0f), mEngine.getWorldWidth(), mEngine.getWorldHeight());
