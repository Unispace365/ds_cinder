...
https_example::https_example()
  : mHttpsRequest(mEngine) {
	...
	registerKeyPress(
		"Paste http(s) request link",
		[this] {
			auto fileNameOrig = ds::Environment::getClipboard();
			mHttpsRequest.makeGetRequest(fileNameOrig, false, false);
		},
		ci::app::KeyEvent::KEY_v, false, true);
	registerKeyPress(
		"Sample post data",
		[this] {
			std::string datay = "{ \"data\": { \"type\": \"collection_links\", \"attributes\": { \"story_type\": "
								"\"Achievement\", \"story_id\": \"13\" }}}";
...
void https_example::setupServer() {
	...
	*/
	...
	descriptionText->setText("See the console for https request output. <br>Press L to post a new request. "
							 "<br>Control-v to test a link with a get request.");
