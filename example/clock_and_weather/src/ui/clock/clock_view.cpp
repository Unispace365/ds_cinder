...
void ClockView::updateClock() {
	...
	auto theD = Poco::DateTimeFormatter::format(ldt, mEngine.getAppSettings().getString("clock:date_format", 0, "%d-%m-%Y %H:%M"));
	auto thet = Poco::DateTimeFormatter::format(ldt, mEngine.getAppSettings().getString("clock:time_format", 0, "%d-%m-%Y %H:%M"));
	if(thet.find("0")  == 0) {
		thet = thet.substr(1);
	}
