	...
	, mVideo(nullptr)
{
	...
	if(meshStyle == "circle") {
		...
		mesh = ci::gl::VboMesh::create(ci::geom::Rect(ci::Rectf(-mParticleScale, -mParticleScale, mParticleScale, mParticleScale)));
	}
...
bool ParticlesView::particleOutOfBounds(Particle& p) {
	if(p.mAge > p.mLifespan || p.mPosition.x < -mParticleScale || p.mPosition.y < -mParticleScale || p.mPosition.x > mEngine.getWorldWidth() + mParticleScale || p.mPosition.y > mEngine.getWorldHeight() + mParticleScale) {
		return true;
	}
	...
	*/
	...
	for(auto& repulsor : mRepulsors) {
		...
		*/
		...
		repulsor.mCenter = ci::vec2(getWidth()/2.0f, getHeight()/2.0f); //::vec2(repSprit->getPosition());
	...
	for(auto it = mParticles.begin(); it < mParticles.end(); ++it) {
		...
		float yN = mPerlin.fBm(ci::vec3(party.mPosition.x + mAnimationCounter * 5.0f, party.mPosition.y, mAnimationCounter) * perlinScale);
