...
MediaViewer::MediaViewer()
  ...
  , mStreamerParent(nullptr) {
	...
	registerKeyPress(
		...
		[this] {
			mNVDecode = !mNVDecode;
			std::cout << "Nvidia decode is " << mNVDecode << std::endl;
		},
	...
	registerKeyPress(
		...
		[this] {
			mGlMode = !mGlMode;
			std::cout << "GL Mode is " << mGlMode << std::endl;
		},
...
void MediaViewer::setupServer() {
	...
	menuItemModels.push_back(ds::ui::TouchMenu::MenuItemModel(L"Exit", "%APP%/data/images/menu/exit_app_normal.png",
															  "%APP%/data/images/menu/exit_app_glow.png",
															  [this](ci::vec3) { ci::app::App::quit(); }));
	menuItemModels.push_back(ds::ui::TouchMenu::MenuItemModel(
		L"Close All", "%APP%/data/images/menu/close_normal.png", "%APP%/data/images/menu/close_glow.png",
		[this](ci::vec3) { mEngine.getNotifier().notify(RequestCloseAllEvent()); }));
	menuItemModels.push_back(ds::ui::TouchMenu::MenuItemModel(L"Search", "%APP%/data/images/menu/search_normal.png",
															  "%APP%/data/images/menu/search_glow.png",
															  [this](ci::vec3 pos) {}));
	menuItemModels.push_back(ds::ui::TouchMenu::MenuItemModel(
		L"Layout", "%APP%/data/images/menu/pinboard_normal.png", "%APP%/data/images/menu/pinboard_glow.png",
		[this](ci::vec3) { mEngine.getNotifier().notify(RequestLayoutEvent()); }));
...
void MediaViewer::onKeyDown(ci::app::KeyEvent event) {
	...
	if (event.getCode() == KeyEvent::KEY_v && event.isControlDown()) {
		...
		if (extensionay.find("png") != std::string::npos || extensionay.find("jpg") != std::string::npos ||
			extensionay.find("jpeg") != std::string::npos || extensionay.find("gif") != std::string::npos) {
		...
		mEngine.getNotifier().notify(RequestMediaOpenEvent(
			newMedia, ci::vec3(mEngine.getWorldWidth() / 2.0f, mEngine.getWorldHeight() / 2.0f, 0.0f), 600.0f));
...
void MediaViewer::fileDrop(ci::app::FileDropEvent event) {
	...
	for (auto it = event.getFiles().begin(); it < event.getFiles().end(); ++it) {
		...
		mEngine.getNotifier().notify(RequestMediaOpenEvent(
			newMedia, ci::vec3(locationy.x - startWidth / 2.0f, locationy.y - startWidth / 2.0f, 0.0f), startWidth,
			mGlMode, mNVDecode));
