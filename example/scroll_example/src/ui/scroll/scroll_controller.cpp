...
ScrollController::ScrollController(ds::ui::SpriteEngine& eng)
  : ds::ui::SmartLayout(eng, "scroll_controller.xml") {
	...
	if (smartScrollList) {
		smartScrollList->setContentItemTappedCallback(
			[this](ds::ui::SmartLayout* item, ds::model::ContentModelRef model) {
				std::cout << "Item tapped id=" << model.getId() << " title=" << model.getPropertyString("title")
						  << std::endl;
			});
