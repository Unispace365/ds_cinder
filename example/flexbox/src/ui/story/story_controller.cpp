...
StoryController::StoryController(ds::ui::SpriteEngine& eng)
  : ds::ui::SmartLayout(eng, "story_controller.xml") {
	...
	listenToEvents<ds::app::IdleStartedEvent>([this](const ds::app::IdleStartedEvent& e) { removeCurrentStory(); });

	/// When an interaction has occurred
	listenToEvents<ds::app::IdleEndedEvent>([this](const ds::app::IdleEndedEvent& e) { setData(); });
...
void StoryController::removeCurrentStory() {
	if (mCurrentStory) {
		/// The animate off script has to finish or this sprite will leak, so be careful not to animate that sprite
		/// again
