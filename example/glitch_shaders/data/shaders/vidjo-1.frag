...
float wow;
float Amount = 1.0;
vec3 iResolution = vec3(3840,2160,1);
...
void main()
{
    ...
    //
    float glitchStrength = (touchPos.x + 55.55)/iResolution.y * 10.0;
	...
	vec4 colSnap = texture( tex0, vec2( px,py) );
	...
	oColor = texture( tex0, vec2( rx,ry) );
