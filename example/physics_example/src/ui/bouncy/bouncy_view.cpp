...
void BouncyView::rebuildBouncies() {
	...
	for (int i = 0; i < numBouncies; i++) {
		...
		sb->setPosition(ci::vec3(ci::randFloat(0.0f, mEngine.getWorldWidth()),
								 ci::randFloat(0.0f, mEngine.getWorldHeight()), 0.0f));
		sb->setLinearVelocity(ci::randFloat(-startVelocity, startVelocity),
							  ci::randFloat(-startVelocity, startVelocity));
